...
### `frontend/src/pages/Faltas.tsx` (492 linhas)

Botões de exportação existentes (linhas ~288-289):
```
📊 Excel   (chama exportarExcel)
📄 PDF     (chama exportarPDF)
```

**Não existe** botão `📋 Folha` — será CRIADO.
**Não existe** função `exportarFolhaOCR()` — será CRIADA.

▸ Nota: o ícone 📋 na linha 285 é do título "📋 Lançamento de Faltas", não um botão.

### `frontend/src/pages/OCR.tsx` (294 linhas)
...
## 4. PASSO 1 — Criar `exportarFolhaOCR()` e botão em `Faltas.tsx`

### 1a — Adicionar botão

**Arquivo:** `frontend/src/pages/Faltas.tsx`
**Localização:** junto dos botões existentes (~linha 289, após `📄 PDF`)

```typescript
<button onClick={exportarFolhaOCR} style={btn('primary', { small: true, outline: true })}>📋 Folha OCR</button>
```

### 1b — Criar função (após `exportarPDF()`, ~linha 249)

**Ação:** Inserir o código abaixo **antes** da função `exportarExcel()` (~linha 251).
...
### 6a — Adicionar estados novos (junto dos outros `useState`, após linha ~68)
...
### 6b — Adicionar toggle de modo na UI (ANTES do bloco de upload)

Localizar a div com `<select>` de Turma e Mês (~linha 162). Inserir o toggle **ANTES** dela:
...
### 6c — Modificar `analisar()` para usar parser conforme modo
...
### 6d — Adicionar função `salvarDiario()` (após a função `salvar()` existente, ~linha 141)
...
### 6e — Adicionar tela de revisão para modo diário

No bloco `step === 'review'` (~linha 195), **adicionar ANTES do `<div style={{ display: 'flex', gap: 8 }}>` dos botões** (~linha 234):
...
### 6g — Adaptar `reiniciar()` para limpar os novos estados
...
|---|---|---|
| `Faltas.tsx` | **ADICIONAR** botão `📋 Folha OCR` | Junto dos botões 📊 Excel e 📄 PDF (~linha 289) |
| `Faltas.tsx` | **CRIAR** função `exportarFolhaOCR()` | Após `exportarPDF()` (~linha 249) — ~100 linhas |
...
| `api.ts` | **NENHUMA** | Usa `upsertFaltasBatch()` já existente |
| `styles.ts` | **NENHUMA** | |
...
2. Clicar **📋 Folha OCR** → janela abre com grade A4 paisagem e células VAZIAS
...
- [ ] **1.** Adicionar botão `📋 Folha OCR` em `Faltas.tsx` (junto dos outros, ~linha 289)
- [ ] **2.** Criar função `exportarFolhaOCR()` em `Faltas.tsx` (~linha 249)
