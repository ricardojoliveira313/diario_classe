...
const checks = [
  ...
  ['turma usa banco interno consolidado com nome completo e folha por RF', page.includes("supabase.from('Turma').select('id,nome,professora,periodo,tipo')") && page.includes("norm(t.professora)!==nome") && page.includes("servidorPorRf.get(dig(freq.rf))")],
  ['período precisa coincidir para atribuir a turma quando informado', page.includes("return !periodoTurma||!periodo||periodoTurma===periodo")],
