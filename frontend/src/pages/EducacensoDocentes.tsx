...
function construirDraft(l:Linha,r:PrepareResponse,educacenso:EducacensoRegistro|null):Draft{
  ...
  const areaPosLocal=(curso:string)=>{const n=norm(curso);const sugerida=n.includes('HISTORIA DA ARTE')||n.includes('ARTE')?'Artes e humanidades':n.includes('PSICOPEDAG')||n.includes('PEDAGOG')||n.includes('EDUCAC')||n.includes('LIBRAS')||n.includes('DOCENCIA')||n.includes('ALFABET')||n.includes('ENSINO')?'Educação':n.includes('TECNOLOG')||n.includes('DIGITAL')||n.includes('TIC')?'Computação e Tecnologias da Informação e Comunicação (TIC)':'';return inOptions(sugerida,areasPos)};
  const posLocal:PosGraduacao[]=posCursos.slice(0,6).map((curso:string,i:number)=>({...posVazia(),tipo:'Especialização',area:areaPosLocal(curso),ano:anosPos[i]||''}));
