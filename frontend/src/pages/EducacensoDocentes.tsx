...
type Draft = {
  ...
  deficiencias: string[]; grauFormacao: string; cursosSuperiores: CursoSuperior[]; possuiPos: boolean;
...
type PrepareResponse = {
  ...
  local?: { profile?: Record<string, any> | null; profileMatch?: string; ficha?: { dados?: Record<string, any> } | null; formacoes?: Array<Record<string, any>> };
};
type EducacensoLookup = { ok: boolean; registro?: EducacensoRegistro | null; match?: string | null; referenciaData?: string | null; arquivoNome?: string | null };
...
const listaPipe = (value: unknown) => {
  const s = txt(value);
  return !s || s === '-' ? [] : s.split('|').map(x => x.trim()).filter(Boolean);
};
...
function formatDataBR(value: string) {
  if (!value) return '';
...
function contemNome(curto: string, completo: string) {
  const a = norm(curto).split(' ').filter(t => t.length > 1); const b = norm(completo).split(' ').filter(Boolean);
  return !!a.length && a.every(t => b.includes(t));
}
function turmasDoServidor(s: Servidor, turmas: Turma[]) {
  const atribuida = norm(s.turma_atribuida || s.sala_atribuida || '');
  if (atribuida) { const porAtribuicao = turmas.filter(t => norm(t.nome) === atribuida); if (porAtribuicao.length) return porAtribuicao; }
  return turmas.filter(t => { const p = txt(t.professora); return !!p && (contemNome(p, s.nome) || contemNome(s.nome, p)); });
}
function modalidadeSugerida(s: Servidor, f: Frequencia, turmas: Turma[]) {
  const cargo = norm(f.cargo || s.cargo); const lotacao = norm(f.lotacao); const nomes = turmas.map(t => norm(t.nome));
...
function splitEndereco(value: string) {
  const s = txt(value); const m = s.match(/^(.*?)[,\s]+(\d+\w?(?:\s*[-/]\s*\w+)?(?:.*)?)$/i);
  return m ? { rua: m[1].trim(), numero: m[2].trim() } : { rua: s, numero: '' };
}
function ufSigla(value: string) {
  const n = norm(value);
  const map: Record<string, string> = { ACRE:'AC',ALAGOAS:'AL',AMAPA:'AP',AMAZONAS:'AM',BAHIA:'BA',CEARA:'CE','DISTRITO FEDERAL':'DF','ESPIRITO SANTO':'ES',GOIAS:'GO',MARANHAO:'MA','MATO GROSSO':'MT','MATO GROSSO DO SUL':'MS','MINAS GERAIS':'MG',PARA:'PA',PARAIBA:'PB',PARANA:'PR',PERNAMBUCO:'PE',PIAUI:'PI','RIO DE JANEIRO':'RJ','RIO GRANDE DO NORTE':'RN','RIO GRANDE DO SUL':'RS',RONDONIA:'RO',RORAIMA:'RR','SANTA CATARINA':'SC','SAO PAULO':'SP',SERGIPE:'SE',TOCANTINS:'TO' };
  const sigla = /^[A-Z]{2}$/.test(n) ? n : (map[n] || ''); return UFS.includes(sigla as any) ? sigla : '';
}
...
function mapCorRaca(value: unknown) {
  if (norm(value) === norm('Não declarado')) return 'Não declarada';
  return inOptions(value, CORES_RACAS);
}
function parseDeficienciasEducacenso(value: unknown) {
  return listaPipe(value).map(v => inOptions(v, DEFICIENCIAS)).filter(Boolean);
}
function parseCursosEducacenso(value: unknown) {
  return listaPipe(value).map(v => inOptions(v, CURSOS_ESPECIFICOS)).filter(Boolean);
}
function parsePosEducacenso(value: unknown, areasPermitidas: readonly string[]) {
  return listaPipe(value).slice(0, 6).map(item => {
    const m = item.match(/^(Especialização|Mestrado|Doutorado)\s*-\s*(.+)$/i);
    if (!m) return null;
    const tipo = inOptions(m[1], TIPOS_POS); const area = inOptions(m[2], areasPermitidas);
    return tipo && area ? { ...posVazia(), tipo, area } : null;
  }).filter((x): x is PosGraduacao => !!x);
}

function construirDraft(l: Linha, r: PrepareResponse, educacenso: EducacensoRegistro | null): Draft {
  const p = r.local?.profile || {}; const f = r.local?.ficha?.dados || {}; const official = r.official || {};
  const endereco = splitEndereco(first(f.endereco, p.endereco)); const formacoes = Array.isArray(r.local?.formacoes) ? r.local!.formacoes! : [];
  const superiores: CursoSuperior[] = formacoes.filter((x: any) => !norm(x.tipo).includes('POS')).slice(0, 3).map((x: any) => ({ ...cursoSuperiorVazio(), tipo: inOptions(x.tipo, TIPOS_SUPERIOR), curso: txt(x.curso), instituicao: txt(x.universidade), ano: txt(x.termino) }));
  if (!superiores.length && txt(f.formacaoPrincipal)) superiores.push({ ...cursoSuperiorVazio(), curso: txt(f.formacaoPrincipal), instituicao: Array.isArray(f.universidades) ? txt(f.universidades[0]) : '' });

  const areasPosOficiais = uniq((r.opcoesOficiais?.pos || []).map(x => x.area));
  const areasPos = areasPosOficiais.length ? areasPosOficiais : [...AREAS_POS];
  ...
  const temSnapshot = !!educacenso;
  const posRawEdu = txt(educacenso?.pos_graduacao_raw);
  const possuiPos = temSnapshot ? (posRawEdu !== '' && posRawEdu !== '-') : true;
  const posGraduacoes = temSnapshot ? posEdu : (posLocal.length ? posLocal : [posVazia()]);

  const etnia = txt(f.etniaReferencia || p.etnia);
  const sexoEdu = inOptions(educacenso?.sexo, SEXOS);
  const corEdu = mapCorRaca(educacenso?.cor_raca_oficial);
  const nacionalidadeEdu = inOptions(educacenso?.nacionalidade_oficial, NACIONALIDADES);
  const grauEdu = inOptions(educacenso?.grau_formacao_oficial, GRAUS);

  return {
    rf: dig(official.rf || l.servidor.rf), nome: first(official.nome, l.servidor.nome), unidadeEscolar: r.unidadeEscolar || SCHOOL,
    modalidade: MODALIDADES.includes(l.modalidade as any) ? l.modalidade : '', cpf: dig(official.cpf || f.cpf || p.cpf),
    dataNasc: first(official.dataNascimento, educacenso?.data_nascimento, f.dataNascimento, p.data_nascimento),
    sexo: sexoEdu, corRaca: corEdu || CORES_RACAS.find(c => norm(c) === norm(etnia)) || '',
    telefone: first(f.telefone1, p.telefone_celular_1, f.telefone2, p.telefone_celular_2, p.telefone_fixo),
    email: first(f.email, p.email), nomeMae: first(f.nomeMae, p.nome_mae), nomePai: first(f.nomePai, p.nome_pai),
    nacionalidade: nacionalidadeEdu || 'Brasileiro(a)', paisEstrangeiro: '', naturalidade: first(f.municipioNascimento, p.municipio_nascimento),
    ufNascimento: ufSigla(first(f.ufNascimento, p.estado_nascimento)), cep: dig(first(f.cep, p.cep)), rua: endereco.rua, numero: endereco.numero,
    ...
    deficiencias: parseDeficienciasEducacenso(educacenso?.deficiencia_raw), grauFormacao: grauEdu,
    cursosSuperiores: superiores.length ? superiores : [cursoSuperiorVazio()], possuiPos,
    posGraduacoes: possuiPos ? (posGraduacoes.length ? posGraduacoes : [posVazia()]) : [],
    cursosEspecificos: parseCursosEducacenso(educacenso?.cursos_especificos_raw),
  };
}
...
function SelectField({ labelText, value, onChange, options, placeholder='Selecione...', disabled=false }: { labelText:string; value:string; onChange:(v:string)=>void; options:readonly string[]; placeholder?:string; disabled?:boolean }) {
  return <label style={{display:'grid',gap:5}}><span style={{color:theme.textSecondary,fontSize:10.5,fontWeight:800}}>{labelText}</span><select style={{...input,width:'100%',opacity:disabled?0.65:1}} value={value} disabled={disabled} onChange={e=>onChange(e.target.value)}><option value="">{placeholder}</option>{options.map(o=><option key={o} value={o}>{o}</option>)}</select></label>;
}
...
export default function EducacensoDocentes() {
  ...
  const [servidores,setServidores]=useState<Servidor[]>([]); const [frequencias,setFrequencias]=useState<Frequencia[]>([]); const [turmas,setTurmas]=useState<Turma[]>([]);
  const [loading,setLoading]=useState(true); const [erro,setErro]=useState(''); const [aviso,setAviso]=useState(''); const [busca,setBusca]=useState('');
  const [token,setToken]=useState(()=>sessionStorage.getItem(TOKEN_KEY)||''); const [expiraEm,setExpiraEm]=useState(()=>sessionStorage.getItem(TOKEN_EXP_KEY)||'');
  const [senha,setSenha]=useState(''); const [desbloqueando,setDesbloqueando]=useState(false); const [sessaoOk,setSessaoOk]=useState(false); const [preparandoRf,setPreparandoRf]=useState('');
  const [linhaEmEdicao,setLinhaEmEdicao]=useState<Linha|null>(null); const [draft,setDraft]=useState<Draft|null>(null); const [fonteMatch,setFonteMatch]=useState('');
  const [fonteEducacenso,setFonteEducacenso]=useState<FonteEducacenso|null>(null); const [confirmacao,setConfirmacao]=useState(false); const [enviando,setEnviando]=useState(false);
  const [resultado,setResultado]=useState<any>(null); const [opcoes,setOpcoes]=useState<OpcoesOficiais>(EMPTY_OPTIONS);

  const bridge = async (body:Record<string,any>, explicitToken?:string) => {
    const t=explicitToken??token; const headers:Record<string,string>={}; if(t)headers['x-censo-token']=t;
    const {data,error}=await supabase.functions.invoke('censo-oficial-bridge',{body,headers});
    if(error){let message=error.message||'Falha na integração direta.';const ctx=(error as any).context;try{if(ctx&&typeof ctx.json==='function'){const detail=await ctx.json();if(detail?.erro)message=detail.erro}}catch{}throw new Error(message)}
    if(!data?.ok)throw new Error(data?.erro||'Operação não autorizada.'); return data;
  };
  const educacensoLookup = async (cpf:string):Promise<EducacensoLookup> => {
    const headers:Record<string,string>={}; if(token)headers['x-censo-token']=token;
    const {data,error}=await supabase.functions.invoke('censo-educacenso-base',{body:{action:'lookup',cpf},headers});
    if(error){let message=error.message||'Falha ao consultar a base Educacenso.';const ctx=(error as any).context;try{if(ctx&&typeof ctx.json==='function'){const detail=await ctx.json();if(detail?.erro)message=detail.erro}}catch{}throw new Error(message)}
    if(!data?.ok)throw new Error(data?.erro||'Falha ao consultar a base Educacenso.'); return data as EducacensoLookup;
  };

  useEffect(()=>{let montado=true;(async()=>{setLoading(true);const[srv,freq,tur]=await Promise.all([
  ...
  useEffect(()=>{if(!token){setSessaoOk(false);return}if(!expiraEm||new Date(expiraEm).getTime()<=Date.now()){sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false);return}let ativo=true;bridge({action:'status'},token).then(()=>{if(ativo)setSessaoOk(true)}).catch(()=>{if(!ativo)return;sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false)});return()=>{ativo=false}},[]);
  ...
  const gradTiposOficiais=uniq(opcoes.graduacao.map(x=>x.tipo)); const gradTipos=gradTiposOficiais.length?gradTiposOficiais:[...TIPOS_SUPERIOR];
  const posAreasOficiais=uniq(opcoes.pos.map(x=>x.area)); const posAreas=posAreasOficiais.length?posAreasOficiais:[...AREAS_POS];
  const instUfsOficiais=uniq(opcoes.instituicoes.map(x=>x.uf)); const instUfs=instUfsOficiais.length?instUfsOficiais:[...UFS];
  ...
  const desbloquear=async()=>{if(!username||!senha){setAviso('Informe novamente sua senha do Diário para liberar a integração direta.');return}setDesbloqueando(true);setAviso('');try{const data=await bridge({action:'auth',username,password:senha},'');sessionStorage.setItem(TOKEN_KEY,data.token);sessionStorage.setItem(TOKEN_EXP_KEY,data.expiraEm);setToken(data.token);setExpiraEm(data.expiraEm);setSessaoOk(true);setSenha('');setAviso('Integração direta segura liberada nesta sessão.')}catch(e:any){setAviso(e?.message||'Não foi possível liberar a integração.')}finally{setDesbloqueando(false)}};
  const bloquear=async()=>{try{if(token)await bridge({action:'logout'})}catch{}sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false);setDraft(null);setLinhaEmEdicao(null);setFonteEducacenso(null);setAviso('Sessão segura do Censo encerrada.')};
  const prepararDireto=async(l:Linha)=>{if(!sessaoOk||!token){setAviso('Desbloqueie primeiro a integração direta segura com sua senha do Diário.');document.getElementById('censo-seguranca')?.scrollIntoView({behavior:'smooth',block:'center'});return}setPreparandoRf(l.servidor.rf);setAviso('');setResultado(null);setConfirmacao(false);setFonteEducacenso(null);try{const data=await bridge({action:'prepare',rf:dig(l.servidor.rf)}) as PrepareResponse;let base:EducacensoLookup|null=null;const cpf=dig(data.official?.cpf||data.local?.ficha?.dados?.cpf||data.local?.profile?.cpf);if(cpf.length===11){try{base=await educacensoLookup(cpf)}catch(e:any){setAviso(`Cadastro preparado, mas a fotografia do Educacenso não pôde ser consultada: ${e?.message||'erro de consulta'}.`)}}setLinhaEmEdicao(l);setOpcoes(data.opcoesOficiais||EMPTY_OPTIONS);setDraft(construirDraft(l,data,base?.registro||null));setFonteMatch(data.local?.profileMatch||'ficha consolidada');if(base?.registro)setFonteEducacenso({referenciaData:base.registro.referencia_data||base.referenciaData||'',arquivoNome:base.registro.arquivo_nome||base.arquivoNome||'',match:base.match||'cpf'});setTimeout(()=>document.getElementById('censo-direto-editor')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}catch(e:any){setAviso(e?.message||'Não foi possível preparar este vínculo.')}finally{setPreparandoRf('')}};
  ...
  const validarDraft=()=>{if(!draft)return['Cadastro não preparado'];const faltam:string[]=[];if(draft.unidadeEscolar!==SCHOOL)faltam.push('Unidade Escolar');if(!draft.modalidade)faltam.push('Modalidade');if(dig(draft.cpf).length!==11)faltam.push('CPF');if(!draft.dataNasc)faltam.push('Data Nasc.');if(!draft.email)faltam.push('E-mail');if(!draft.grauFormacao)faltam.push('Grau de Formação');return faltam};
  const enviarDireto=async()=>{if(!draft||!linhaEmEdicao)return;const faltam=validarDraft();if(faltam.length){setAviso(`Antes de enviar, revise: ${faltam.join(', ')}.`);return}if(!confirmacao){setAviso('Marque a declaração de conferência antes do envio oficial.');return}setEnviando(true);setAviso('');setResultado(null);try{const data=await bridge({action:'submit',dados:draft,confirmacaoFinal:true,requestId:crypto.randomUUID()});setResultado(data.result??data);setConfirmacao(false);setAviso('Envio concluído pelo canal direto. O retorno oficial foi registrado para auditoria.')}catch(e:any){setAviso(e?.message||'Falha no envio. Nada deve ser reenviado sem conferir o aviso.')}finally{setEnviando(false)}};

  if(loading)return <Loading/>;
  const expiraLabel=expiraEm?new Date(expiraEm).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):''; const faltamDraft=draft?validarDraft():[];
  ...
  return <div style={{marginTop:4}}>
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:20,marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div><h1 style={{margin:0,color:theme.text,fontSize:24}}>👩‍🏫 Censo Docentes 2026</h1><p style={{color:theme.textSecondary,margin:'7px 0 0',maxWidth:900,lineHeight:1.55}}>Campos e listas dinâmicas espelhados do formulário oficial, com pré-preenchimento pela fotografia mais recente do Educacenso e conferência antes do envio.</p></div><span style={{color:sessaoOk?theme.success:theme.warning,fontWeight:850,fontSize:11}}>{sessaoOk?'🔐 Integração direta segura ativa':'🔒 Integração direta bloqueada'}</span></div>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:15}}><a href={FORM_URL} target="_blank" rel="noopener noreferrer" style={{...btn('ghost'),textDecoration:'none'}}>Abrir formulário oficial</a></div>
    ...
    <section id="censo-seguranca" style={{background:theme.card,border:`1px solid ${sessaoOk?theme.success:theme.warning}55`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:15,marginBottom:14}}><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}><div><div style={{color:theme.text,fontWeight:900,fontSize:14}}>🔐 Canal direto protegido</div><div style={{color:theme.textSecondary,fontSize:11.5,marginTop:4}}>Busca por RF, opções oficiais em tempo real, base histórica Educacenso e confirmação humana final.</div></div>{sessaoOk?<div style={{display:'flex',gap:8,alignItems:'center'}}><span style={{color:theme.success,fontSize:11.5,fontWeight:800}}>Ativa até {expiraLabel}</span><button style={btn('ghost',{small:true})} onClick={bloquear}>Encerrar sessão segura</button></div>:<div style={{display:'flex',gap:7,alignItems:'center',flexWrap:'wrap'}}><input type="password" autoComplete="current-password" style={{...input,width:220}} value={senha} onChange={e=>setSenha(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void desbloquear()}} placeholder="Senha do Diário"/><button style={btn('primary')} disabled={desbloqueando||!senha} onClick={desbloquear}>{desbloqueando?'Liberando...':'Liberar integração direta'}</button></div>}</div></section>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}><div><h2 style={{margin:0,color:theme.text,fontSize:19}}>Conferência oficial — {draft.nome}</h2><div style={{marginTop:5,color:theme.textSecondary,fontSize:11.5}}>RF <strong>{formatRf(draft.rf)}</strong> · CPF {maskCpf(draft.cpf)} · fonte local: {fonteMatch||'consolidada'}{fonteEducacenso&&<> · <strong style={{color:theme.success}}>Educacenso {formatDataBR(fonteEducacenso.referenciaData)} ({fonteEducacenso.match.toUpperCase()})</strong></>}</div></div><button style={btn('ghost',{small:true})} onClick={()=>{setDraft(null);setLinhaEmEdicao(null);setResultado(null);setFonteEducacenso(null)}}>Fechar</button></div>
      <div style={{marginTop:11,padding:10,borderRadius:8,background:`${theme.warning}0F`,border:`1px solid ${theme.warning}55`,color:theme.textSecondary,fontSize:11.5,lineHeight:1.5}}><strong style={{color:theme.text}}>Conferência humana:</strong> os campos autodeclarados continuam editáveis. Os asteriscos reproduzem os campos que o próprio formulário oficial bloqueia como obrigatórios no envio.</div>
      {fonteEducacenso&&<div style={{marginTop:8,padding:10,borderRadius:8,background:`${theme.success}0F`,border:`1px solid ${theme.success}55`,color:theme.textSecondary,fontSize:11.5,lineHeight:1.5}}><strong style={{color:theme.success}}>✓ Base Educacenso aplicada:</strong> quando disponíveis no snapshot de {formatDataBR(fonteEducacenso.referenciaData)}, Sexo, Cor/Raça, Nacionalidade, Grau de Formação, Pós-Graduação e cursos de 80h foram pré-preenchidos. Todos permanecem editáveis para conferência.</div>}
      ...
      <div style={{marginTop:13}}><label style={{display:'flex',gap:7,alignItems:'center',color:theme.textSecondary,fontSize:11.5}}><input type="checkbox" checked={!draft.possuiPos} onChange={e=>{const possui=!e.target.checked;patch('possuiPos',possui);if(possui&&draft.posGraduacoes.length===0)patch('posGraduacoes',[posVazia()])}}/>Não possui pós-graduação concluída</label></div>
      {draft.possuiPos&&<div style={{display:'grid',gap:8,marginTop:8}}>{draft.posGraduacoes.map((c,i)=><div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:8,padding:10,background:'var(--ghost-bg)'}}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:7}}><SelectField labelText="Tipo" value={c.tipo} onChange={v=>atualizarPos(i,{tipo:v})} options={TIPOS_POS}/><SelectField labelText="Área" value={c.area} onChange={v=>atualizarPos(i,{area:v})} options={posAreas}/><Field labelText="Ano Conclusão" value={c.ano} onChange={v=>atualizarPos(i,{ano:v})}/></div><label style={{display:'flex',gap:7,alignItems:'center',color:theme.textSecondary,fontSize:11,marginTop:8}}><input type="checkbox" checked={c.entregue} onChange={e=>atualizarPos(i,{entregue:e.target.checked})}/>Cópia entregue nesta UE</label><button style={{...btn('ghost',{small:true}),marginTop:7}} onClick={()=>patch('posGraduacoes',draft.posGraduacoes.filter((_,idx)=>idx!==i))}>Remover pós</button></div>)}{draft.posGraduacoes.length<6&&<button style={btn('ghost',{small:true})} onClick={()=>patch('posGraduacoes',[...draft.posGraduacoes,posVazia()])}>+ Adicionar Pós-Graduação</button>}</div>}
