...
export default function EducacensoDocentes(){
  ...
  const[resultado,setResultado]=useState<{rf:string;valor:any}|null>(null),[opcoes,setOpcoes]=useState<OpcoesOficiais>(EMPTY_OPTIONS),[envios,setEnvios]=useState<EnvioStatus[]>([]);
  ...
  const filtradas=useMemo(()=>{const q=norm(busca);const correspondentes=q?linhas.filter(l=>norm([l.servidor.nome,l.servidor.rf,l.frequencia.cargo,l.frequencia.lotacao,l.turmas.map(t=>t.nome).join(' '),l.modalidade].join(' ')).includes(q)):linhas;const alfabetica=(a:Linha,b:Linha)=>a.servidor.nome.localeCompare(b.servidor.nome,'pt-BR',{sensitivity:'base'})||dig(a.servidor.rf).localeCompare(dig(b.servidor.rf));return{pendentes:correspondentes.filter(l=>!enviosPorRf.has(dig(l.servidor.rf))).sort(alfabetica),processadas:correspondentes.filter(l=>enviosPorRf.has(dig(l.servidor.rf))).sort(alfabetica)}},[linhas,busca,enviosPorRf]);
  ...
  const enviarDireto=async()=>{if(!draft||!linhaEmEdicao||!competencia)return;setEnviando(true);setAviso('');setResultado(null);const rfEnviado=dig(draft.rf);try{const data=await bridge({action:'submit',dados:draft,competencia,requestId:crypto.randomUUID()});setResultado({rf:rfEnviado,valor:data.result??data});await carregarEnvios();setAviso('Envio concluído pelo canal direto. O retorno oficial foi registrado para auditoria.')}catch(e:any){setAviso(e?.message||'Falha no envio. Nada deve ser reenviado sem conferir o aviso.')}finally{setEnviando(false)}};
  ...
  return<div style={{marginTop:4}}>
    ...
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:14,marginBottom:14}}>
      <div style={{color:theme.text,fontWeight:900,fontSize:14}}>📋 Fluxo oficial pela folha de frequência</div>
      <div style={{color:theme.textSecondary,fontSize:11.5,marginTop:5,lineHeight:1.5}}>A lista abaixo é formada exclusivamente pelos vínculos docentes da folha de frequência da competência selecionada. O Diário apenas prepara e confere o cadastro do RF escolhido. <strong>Não existe envio automático em lote.</strong> O envio só acontece quando você abrir o professor correto e clicar pessoalmente em “Salvar e enviar ao formulário oficial”.</div>
    </section>
    ...
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1000}}><thead><tr style={{background:'var(--ghost-bg)'}}>{['Professor','RF','Frequência','Período','Turma/atuação','Modalidade sugerida','Ação'].map(h=><th key={h} style={{padding:'11px 8px',textAlign:'left',color:theme.textSecondary,fontSize:10.5,borderBottom:`1px solid ${theme.border}`}}>{h}</th>)}</tr></thead>{[
      {chave:'pendentes',titulo:'📝 A preparar',descricao:'Formulários ainda não enviados',linhas:filtradas.pendentes,cor:theme.warning},
      {chave:'processadas',titulo:'✅ Enviados',descricao:'Formulários enviados ou já processados',linhas:filtradas.processadas,cor:theme.success},
    ].map(grupo=><tbody key={grupo.chave}><tr><td colSpan={7} style={{padding:'10px 12px',background:`${grupo.cor}12`,borderTop:`2px solid ${grupo.cor}55`,borderBottom:`1px solid ${grupo.cor}44`,color:grupo.cor,fontSize:12,fontWeight:950}}>{grupo.titulo} <span style={{fontWeight:750,color:theme.textSecondary}}>· {grupo.linhas.length} · {grupo.descricao}</span></td></tr>{grupo.linhas.length===0?<tr><td colSpan={7} style={{padding:'14px 12px',color:theme.textMuted,fontSize:11}}>Nenhum professor neste grupo.</td></tr>:grupo.linhas.map((l,idx)=>{const enviosLinha=enviosDaLinha(l),envio=enviosLinha[0]||null;const enviadoOk=envio?.status==='enviado',revisao=envio?.status==='revisao_necessaria',existente=envio?.status==='confirmado_secretaria';const corStatus=enviadoOk?theme.success:revisao?theme.danger:existente?theme.warning:theme.borderLight;return <tr key={`${l.servidor.rf}-${l.servidor.nome}`} style={{borderBottom:`1px solid ${corStatus}`,background:envio?`${corStatus}12`:(idx%2===0?'var(--row-even)':'var(--row-odd)')}}><td style={{padding:'10px 8px',fontSize:12,color:theme.text}}><strong>{l.servidor.nome}</strong>{envio&&<div style={{color:corStatus,fontSize:10,fontWeight:900,marginTop:3}}>{enviadoOk?'✅ ENVIADO':revisao?'⚠️ REVISÃO NECESSÁRIA':'⚠️ CADASTRO EXISTE NA SECRETARIA — CONTEÚDO NÃO VALIDADO'} · {formatDataHora(envio.criadoEm)}{envio.modalidade?` · ${envio.modalidade}`:''}</div>}<div style={{color:theme.textMuted,fontSize:9.5,marginTop:3}}>{l.frequencia.cargo||l.servidor.cargo}</div>{l.vinculosMesmoNome>1&&<div style={{color:theme.warning,fontSize:9.5,fontWeight:800}}>⚠️ {l.vinculosMesmoNome} RFs — tratar por vínculo</div>}{!l.cruzamentoRfOk&&<div style={{color:theme.warning,fontSize:9.5,fontWeight:800}}>⚠️ RF da frequência não localizado na base de professores — conferir</div>}{l.cruzamentoRfOk&&!l.cruzamentoNomeOk&&<div style={{color:theme.warning,fontSize:9.5,fontWeight:800}}>⚠️ RF confere, mas o nome diverge entre as bases — conferir</div>}</td><td style={{padding:'10px 8px',fontWeight:850,color:theme.text}}>{formatRf(l.servidor.rf)}</td><td style={{padding:'10px 8px',color:theme.success,fontSize:11}}>✓ pág. {l.frequencia.pagina_pdf??'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.servidor.periodo||'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.turmas.length?l.turmas.map(t=>t.nome).join(', '):'⚠️ A conferir'}</td><td style={{padding:'10px 8px',color:l.modalidade?theme.textSecondary:theme.warning,fontSize:11}}>{l.modalidade||'A conferir'}</td><td style={{padding:'10px 8px'}}>{envio?<div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}><span style={{padding:'6px 9px',borderRadius:999,background:`${corStatus}18`,border:`1px solid ${corStatus}55`,color:corStatus,fontWeight:900,fontSize:10.5}}>{enviadoOk?'✅ ENVIADO':revisao?'⚠️ REVISÃO NECESSÁRIA':'⚠️ JÁ CADASTRADO — NÃO VALIDADO'}</span>{revisao&&<span style={{color:theme.danger,fontSize:10,fontWeight:800}}>{envio.motivoRevisao||'Conteúdo informado como incorreto.'}</span>}{existente&&<span title={envio.confirmacaoMensagem||'A Secretaria informa apenas que o cadastro existe.'} style={{color:theme.warning,fontSize:10}}>A existência foi confirmada; o conteúdo não foi validado.</span>}{enviosLinha.filter(e=>e.status==='enviado'&&e.comprovanteUrl).map((e,i)=><a key={`${e.modalidade}-${e.criadoEm}`} href={e.comprovanteUrl} target="_blank" rel="noopener noreferrer" style={{...btn('ghost',{small:true}),textDecoration:'none'}}>📄 Comprovante {enviosLinha.length>1?i+1:''}</a>)}<button style={btn('ghost',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>Revisar formulário</button></div>:<button style={btn('primary',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>{preparandoRf===l.servidor.rf?'Preparando...':'Preparar formulário'}</button>}</td></tr>})}</tbody>)}</table></div></section>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      ...
      {draft.deficiencias.length===0&&<div style={{marginBottom:8,padding:'8px 10px',borderRadius:8,background:`${theme.warning}0F`,border:`1px solid ${theme.warning}66`,color:theme.warning,fontSize:11.5,fontWeight:850}}>⚠️ Nenhuma opção foi assinalada nesta seção. Confira e marque a condição correta, inclusive “Não possuo deficiência”, quando for o caso. Este aviso não bloqueia o envio.</div>}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:6}}>{DEFICIENCIAS.map(d=><label key={d} style={{display:'flex',alignItems:'center',gap:7,padding:'7px 8px',borderRadius:7,border:`1px solid ${draft.deficiencias.length===0?theme.warning:theme.border}`,color:theme.textSecondary,fontSize:11.5,cursor:'pointer'}}><input type="checkbox" checked={draft.deficiencias.includes(d)} onChange={()=>toggleDef(d)}/>{d}</label>)}</div>
      ...
      <div style={{marginTop:15,padding:12,borderRadius:8,border:`1px solid ${theme.success}55`,background:`${theme.success}0F`}}><div style={{color:theme.success,fontSize:11.5,fontWeight:850}}>✓ Dados copiados das fontes disponíveis para sua conferência.</div><div style={{color:theme.textSecondary,fontSize:10.5,marginTop:4}}>O sistema não envia este formulário sozinho. O envio abaixo ocorre somente pelo seu clique neste professor.</div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}><button style={btn('success')} disabled={enviando} onClick={enviarDireto}>{enviando?'Enviando...':'✅ Salvar e enviar ao formulário oficial'}</button><a href={FORM_URL} target="_blank" rel="noopener noreferrer" style={{...btn('ghost'),textDecoration:'none'}}>Abrir formulário manual</a></div></div>
      {resultado&&dig(resultado.rf)===dig(draft.rf)&&<div style={{marginTop:12,padding:12,borderRadius:8,border:`1px solid ${theme.success}55`,background:`${theme.success}10`,color:theme.textSecondary,fontSize:11.5}}><strong style={{color:theme.success}}>✓ Retorno oficial recebido.</strong>{typeof resultado.valor==='string'&&/^https?:\/\//i.test(resultado.valor)?<div style={{marginTop:7}}><a href={resultado.valor} target="_blank" rel="noopener noreferrer" style={{color:theme.primaryText,fontWeight:800}}>Abrir comprovante/PDF oficial</a></div>:<div style={{marginTop:7}}>O retorno foi registrado na auditoria interna.</div>}</div>}
    </section>}
  </div>;
}
