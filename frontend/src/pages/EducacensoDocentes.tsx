...
function montarSuperiores(r:PrepareResponse){
  ...
  if(formacoes.length){
    return formacoes.filter((x:any)=>!norm(x.tipo).includes('POS')).slice(0,3).map((x:any)=>{
      const grad=resolveGraduacao(txt(x.curso),opcoes);const inst=resolveInstituicao(txt(x.universidade),opcoes);
      return{...cursoSuperiorVazio(),tipo:grad?.tipo||inOptions(x.tipo,TIPOS_SUPERIOR),area:grad?.area||'',curso:grad?.curso||'',ano:anoConclusao(x.termino),...(inst||{})};
    });
  }
...
export default function EducacensoDocentes(){
  ...
  const[fonteEducacenso,setFonteEducacenso]=useState<FonteEducacenso|null>(null),[refFicha,setRefFicha]=useState<ReferenciaFicha|null>(null),[confirmacao,setConfirmacao]=useState(false),[enviando,setEnviando]=useState(false);
  ...
  const enviosMap=useMemo(()=>{const m=new Map<string,EnvioStatus>();for(const e of envios)m.set(`${dig(e.rf)}|||${e.modalidade}`,e);return m},[envios]);
  const envioDaLinha=(l:Linha)=>enviosMap.get(`${dig(l.servidor.rf)}|||${l.modalidade}`)||null;
  const totalEnviados=useMemo(()=>linhas.filter(l=>!!enviosMap.get(`${dig(l.servidor.rf)}|||${l.modalidade}`)).length,[linhas,enviosMap]);
  ...
  const bloquear=async()=>{try{if(token)await bridge({action:'logout'})}catch{}sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false);setEnvios([]);setDraft(null);setLinhaEmEdicao(null);setFonteEducacenso(null);setRefFicha(null);setAviso('Sessão segura do Censo encerrada.')};
  const prepararDireto=async(l:Linha)=>{if(!sessaoOk||!token){setAviso('Desbloqueie primeiro a integração direta segura com sua senha do Diário.');document.getElementById('censo-seguranca')?.scrollIntoView({behavior:'smooth',block:'center'});return}setPreparandoRf(l.servidor.rf);setAviso('');setResultado(null);setConfirmacao(false);setFonteEducacenso(null);setRefFicha(null);try{const data=await bridge({action:'prepare',rf:dig(l.servidor.rf)}) as PrepareResponse;let base:EducacensoLookup|null=null;const cpf=dig(data.official?.cpf||data.local?.ficha?.dados?.cpf||data.local?.profile?.cpf);if(cpf.length===11){try{base=await educacensoLookup(cpf)}catch(e:any){setAviso(`Cadastro preparado, mas a fotografia do Educacenso não pôde ser consultada: ${e?.message||'erro de consulta'}.`)}}setLinhaEmEdicao(l);setOpcoes(data.opcoesOficiais||EMPTY_OPTIONS);setDraft(construirDraft(l,data,base?.registro||null));setFonteMatch(data.local?.profileMatch||'ficha consolidada');setRefFicha(referenciaFicha(data));if(base?.registro)setFonteEducacenso({referenciaData:base.registro.referencia_data||base.referenciaData||'',arquivoNome:base.registro.arquivo_nome||base.arquivoNome||'',match:base.match||'cpf'});setTimeout(()=>document.getElementById('censo-direto-editor')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}catch(e:any){setAviso(e?.message||'Não foi possível preparar este vínculo.')}finally{setPreparandoRf('')}};
  ...
  const enviarDireto=async()=>{if(!draft||!linhaEmEdicao)return;const faltam=validarDraft();if(faltam.length){setAviso(`Antes de enviar, revise: ${faltam.join(', ')}.`);return}if(!confirmacao){setAviso('Marque a declaração de conferência antes do envio oficial.');return}setEnviando(true);setAviso('');setResultado(null);try{const data=await bridge({action:'submit',dados:draft,confirmacaoFinal:true,requestId:crypto.randomUUID()});setResultado(data.result??data);await carregarEnvios();setConfirmacao(false);setAviso('Envio concluído pelo canal direto. O retorno oficial foi registrado para auditoria.')}catch(e:any){setAviso(e?.message||'Falha no envio. Nada deve ser reenviado sem conferir o aviso.')}finally{setEnviando(false)}};
  ...
  return<div style={{marginTop:4}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:9,marginBottom:14}}><Resumo titulo="Folhas de frequência" valor={frequencias.length} cor={theme.primaryText} sub="Setembro/2026"/><Resumo titulo="Vínculos docentes" valor={linhas.length} cor={theme.success} sub="Base principal"/><Resumo titulo="Enviados" valor={totalEnviados} cor={theme.success} sub={`${totalEnviados}/${linhas.length} vínculos`}/><Resumo titulo="Fora da frequência" valor={foraDaFrequencia.length} cor={foraDaFrequencia.length?theme.warning:theme.success} sub="Cadastro ativo, sem folha"/></div>
    ...
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1000}}><thead><tr style={{background:'var(--ghost-bg)'}}>{['Professor','RF','Frequência','Período','Turma/atuação','Modalidade sugerida','Ação'].map(h=><th key={h} style={{padding:'11px 8px',textAlign:'left',color:theme.textSecondary,fontSize:10.5,borderBottom:`1px solid ${theme.border}`}}>{h}</th>)}</tr></thead><tbody>{filtradas.map((l,idx)=>{const envio=envioDaLinha(l);return <tr key={`${l.servidor.rf}-${l.servidor.nome}`} style={{borderBottom:`1px solid ${envio?theme.success:theme.borderLight}`,background:envio?`${theme.success}12`:(idx%2===0?'var(--row-even)':'var(--row-odd)')}}><td style={{padding:'10px 8px',fontSize:12,color:theme.text}}><strong>{l.servidor.nome}</strong>{envio&&<div style={{color:theme.success,fontSize:10,fontWeight:900,marginTop:3}}>✅ ENVIADO · {formatDataHora(envio.criadoEm)}</div>}<div style={{color:theme.textMuted,fontSize:9.5,marginTop:3}}>{l.frequencia.cargo||l.servidor.cargo}</div>{l.vinculosMesmoNome>1&&<div style={{color:theme.warning,fontSize:9.5,fontWeight:800}}>⚠️ {l.vinculosMesmoNome} RFs — tratar por vínculo</div>}</td><td style={{padding:'10px 8px',fontWeight:850,color:theme.text}}>{formatRf(l.servidor.rf)}</td><td style={{padding:'10px 8px',color:theme.success,fontSize:11}}>✓ pág. {l.frequencia.pagina_pdf??'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.servidor.periodo||'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.turmas.length?l.turmas.map(t=>t.nome).join(', '):'⚠️ conferir'}</td><td style={{padding:'10px 8px',color:l.modalidade?theme.textSecondary:theme.warning,fontSize:11}}>{l.modalidade||'conferir'}</td><td style={{padding:'10px 8px'}}>{envio?<div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}><span style={{padding:'6px 9px',borderRadius:999,background:`${theme.success}18`,border:`1px solid ${theme.success}55`,color:theme.success,fontWeight:900,fontSize:10.5}}>✅ ENVIADO</span>{envio.comprovanteUrl&&<a href={envio.comprovanteUrl} target="_blank" rel="noopener noreferrer" style={{...btn('ghost',{small:true}),textDecoration:'none'}}>📄 Ver comprovante</a>}<button style={btn('ghost',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>Revisar</button></div>:<button style={btn('primary',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>{preparandoRf===l.servidor.rf?'Preparando...':'Preparar direto'}</button>}</td></tr>})}</tbody></table></div></section>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}><div><h2 style={{margin:0,color:theme.text,fontSize:19}}>Conferência oficial — {draft.nome}</h2><div style={{marginTop:5,color:theme.textSecondary,fontSize:11.5}}>RF <strong>{formatRf(draft.rf)}</strong> · CPF {maskCpf(draft.cpf)} · fonte local: {fonteMatch||'consolidada'}{fonteEducacenso&&<> · <strong style={{color:theme.success}}>Educacenso {formatDataBR(fonteEducacenso.referenciaData)} ({fonteEducacenso.match.toUpperCase()})</strong></>}</div></div><button style={btn('ghost',{small:true})} onClick={()=>{setDraft(null);setLinhaEmEdicao(null);setResultado(null);setFonteEducacenso(null);setRefFicha(null)}}>Fechar</button></div>
      ...
      {fonteEducacenso&&<div style={{marginTop:8,padding:10,borderRadius:8,background:`${theme.success}0F`,border:`1px solid ${theme.success}55`,color:theme.textSecondary,fontSize:11.5,lineHeight:1.5}}><strong style={{color:theme.success}}>✓ Base Educacenso aplicada:</strong> Sexo, Cor/Raça, Nacionalidade, Grau de Formação, Pós-Graduação e cursos de 80h são aproveitados quando presentes na fotografia de {formatDataBR(fonteEducacenso.referenciaData)}.</div>}
