...
type PrepareResponse = {
  ...
  local?: {
    ...
    mestre?:{ ano?:number; rf?:string; nome?:string; fonte_arquivo?:string; fonte_timestamp?:string; dados?:Record<string,any> }|null;
  };
};
...
type EnvioStatus = { rf:string; modalidade:string; status:'enviado'|'confirmado_secretaria'|'revisao_necessaria'; criadoEm:string; comprovanteUrl:string; confirmadoSecretaria?:boolean; confirmacaoMensagem?:string; conteudoValidado?:boolean; motivoRevisao?:string };
...
function parseDeficienciasEducacenso(value:unknown){return listaPipe(value)}
function parseCursosEducacenso(value:unknown){return listaPipe(value)}
function parsePosEducacenso(value:unknown,_areasPermitidas:readonly string[]){return listaPipe(value).slice(0,6).map(item=>{const m=item.match(/^(.+?)\s*-\s*(.+)$/);return m?{...posVazia(),tipo:txt(m[1]),area:txt(m[2])}:null}).filter((x):x is PosGraduacao=>!!x)}
...
const IDADE_MINIMA_CONCLUSAO=15;
...
function simPlanilha(value:unknown){return ['SIM','S','TRUE','1','X'].includes(norm(value))}
function itensPlanilha(value:unknown,options:readonly string[]){
  const raw=txt(value);if(!raw)return[];
  const partes=raw.split(/\s*[|;,]\s*/).map(x=>x.trim()).filter(Boolean);
  return partes.map(x=>inOptions(x,options)||x).filter(Boolean);
}
function cursosMestre(dados:Record<string,any>,_opcoes:OpcoesOficiais):CursoSuperior[]{
  const cursos:CursoSuperior[]=[];
  for(let i=1;i<=3;i++){
    const tipo=txt(dados[`${i}º Tipo`]),area=txt(dados[`${i}º Área`]);
    const cursoRaw=first(dados[`${i}º Curso — sugestão Censo`],dados[`${i}º Curso — declarado na ficha`]);
    const instituicaoRaw=txt(dados[`${i}º Instituição`]);
    const categoriaRaw=txt(dados[`${i}º Categoria/Org.`]);
    const item:CursoSuperior={
      ...cursoSuperiorVazio(),tipo,area,curso:cursoRaw,
      ano:txt(dados[`${i}º Ano Conclusão`]),ufInstituicao:txt(dados[`${i}º UF Instituição`]),
      categoriaOrg:categoriaRaw,instituicao:instituicaoRaw,
      entregue:simPlanilha(dados[`${i}º Cópia entregue UE`]),
    };
    if(tipo||area||cursoRaw)cursos.push(item);
  }
  return cursos;
}
function posMestre(dados:Record<string,any>):PosGraduacao[]{
  const itens:PosGraduacao[]=[];
  for(let i=1;i<=6;i++){
    const item:PosGraduacao={tipo:txt(dados[`Pós ${i} — Tipo`]),area:txt(dados[`Pós ${i} — Área`]),ano:txt(dados[`Pós ${i} — Ano`]),entregue:simPlanilha(dados[`Pós ${i} — Cópia entregue UE`])};
    if(item.tipo||item.area||item.ano)itens.push(item);
  }
  return itens;
}
function situacaoPosMestre(value:unknown):boolean|null{
  const n=norm(value);if(!n)return null;if(n.includes('NAO POSSUI')||n.includes('SEM POS'))return false;if(n.includes('POSSUI'))return true;return null;
}
...
function construirDraft(l:Linha,r:PrepareResponse,base:EducacensoLookup|null):{draft:Draft;avisoIdade:string}{
  ...
  const p=r.local?.profile||{},f=r.local?.ficha?.dados||{},official=r.official||{},m=r.local?.mestre?.dados||{};
  ...
  const superioresMestre=temMestre?cursosMestre(m,r.opcoesOficiais||EMPTY_OPTIONS):[];
  const superioresBrutos=superioresMestre.length?superioresMestre:montarSuperiores(r);
  ...
  const possuiPosPlanilha=temMestre?situacaoPosMestre(m['Situação da Pós-Graduação']):null;
  const temPosNaPlanilha=temMestre&&(temPosEstruturada||possuiPosPlanilha!==null);
  const possuiPos:boolean|null=temPosNaPlanilha?(temPosEstruturada?true:possuiPosPlanilha):temIndicacaoPosEducacenso?true:possuiPosFicha!==null?possuiPosFicha:posRawEdu==='-'?false:null;
  const posGraduacoesBrutas=temPosNaPlanilha?posEstruturada:temPosEducacenso?posEdu:possuiPos===true?[posVazia()]:[];
  const superioresCheck=superioresMestre.length?{itens:superioresBrutos,descartados:0}:limparAnosImplausiveis(superioresBrutos,dataNasc);
  const posCheckIdade=temPosNaPlanilha?{itens:posGraduacoesBrutas,descartados:0}:limparAnosImplausiveis(posGraduacoesBrutas,dataNasc);
  const posCheckCronologia=temPosNaPlanilha?{pos:posCheckIdade.itens,descartados:0}:limparCronologiaPos(superioresCheck.itens,posCheckIdade.itens);
  ...
  const etnia=txt(f.etniaReferencia||p.etnia),nacionalidadeEdu=txt(educacenso?.nacionalidade_oficial),grauEdu=txt(educacenso?.grau_formacao_oficial);
  const grauLocal=superiores.some(c=>c.curso)?'Ensino Superior':norm(f.ensinoMedioMagisterio).includes('MAGISTERIO')?'Magistério':'';
  const draft:Draft={
    rf:dig(official.rf||m.RF||l.servidor.rf),nome:first(temMestre?m['Nome Completo']:'',official.nome,l.servidor.nome),unidadeEscolar:first(temMestre?m['Unidade Escolar']:'',r.unidadeEscolar,SCHOOL),
    modalidade:first(temMestre?m['Modalidade sugerida']:'',l.modalidade,modalidadeSugerida(l.servidor,l.frequencia,l.turmas)),cpf:dig(first(temMestre?m.CPF:'',official.cpf,f.cpf,p.cpf)),
    dataNasc,sexo:first(temMestre?m.Sexo:'',educacenso?.sexo),
    corRaca:first(temMestre?m['Cor/Raça']:'',educacenso?.cor_raca_oficial,CORES_RACAS.find(c=>norm(c)===norm(etnia))),telefone:first(temMestre?m['Telefone/Celular']:'',f.telefone1,p.telefone_celular_1,f.telefone2,p.telefone_celular_2,p.telefone_fixo),
    email:first(temMestre?m['E-mail']:'',f.email,p.email),nomeMae:first(temMestre?m['Filiação 1']:'',f.nomeMae,p.nome_mae),nomePai:first(temMestre?m['Filiação 2']:'',f.nomePai,p.nome_pai),nacionalidade:first(temMestre?m.Nacionalidade:'',educacenso?.nacionalidade_oficial,nacionalidadeEdu,'Brasileiro(a)'),paisEstrangeiro:'',
    naturalidade:first(temMestre?m['Naturalidade (Cidade)']:'',f.municipioNascimento,p.municipio_nascimento),ufNascimento:ufSigla(first(temMestre?m['UF Nascimento']:'',f.ufNascimento,p.estado_nascimento)),cep:dig(first(temMestre?m.CEP:'',f.cep,p.cep)),rua:endereco.rua,numero:endereco.numero,
    bairro:first(temMestre?m.Bairro:'',f.bairro,p.bairro),municipio:first(temMestre?m['Município']:'',f.municipio,p.municipio,'Santo André'),uf:ufSigla(first(temMestre?m.UF:'',f.estado,p.estado,'SP'))||'SP',
    deficiencias:txt(temMestre?m['Deficiência/TEA/Altas Habilidades']:'')?itensPlanilha(m['Deficiência/TEA/Altas Habilidades'],DEFICIENCIAS):parseDeficienciasEducacenso(educacenso?.deficiencia_raw),grauFormacao:first(temMestre?m['Maior Grau de Formação']:'',educacenso?.grau_formacao_oficial,grauEdu,grauLocal),cursosSuperiores:superiores.length?superiores:[cursoSuperiorVazio()],possuiPos,
    posGraduacoes:possuiPos?(posGraduacoes.length?posGraduacoes:[posVazia()]):[],cursosEspecificos:txt(temMestre?m['Cursos específicos 80h']:'')?itensPlanilha(m['Cursos específicos 80h'],CURSOS_ESPECIFICOS):parseCursosEducacenso(educacenso?.cursos_especificos_raw),
  };
  return{draft,avisoIdade};
}

function auditarDraftParaLote(d:Draft,avisoIdade=''){
  const pendencias:string[]=[];
  const exigir=(ok:boolean,label:string)=>{if(!ok)pendencias.push(label)};
  exigir(!!txt(d.unidadeEscolar),'Unidade Escolar');
  exigir(!!txt(d.modalidade),'Modalidade');
  exigir(dig(d.cpf).length===11,'CPF');
  exigir(/^\d{4}-\d{2}-\d{2}$/.test(txt(d.dataNasc)),'Data de nascimento');
  exigir(!!txt(d.email)&&txt(d.email).includes('@'),'E-mail');
  exigir(!!txt(d.sexo),'Sexo');
  exigir(!!txt(d.corRaca),'Cor/Raça');
  exigir(!!txt(d.telefone),'Telefone/Celular');
  exigir(!!txt(d.nomeMae),'Filiação 1');
  exigir(!!txt(d.nacionalidade),'Nacionalidade');
  exigir(!!txt(d.naturalidade),'Naturalidade');
  exigir(!!txt(d.ufNascimento),'UF de nascimento');
  exigir(!!txt(d.cep),'CEP');
  exigir(!!txt(d.rua),'Rua/Avenida');
  exigir(!!txt(d.bairro),'Bairro');
  exigir(!!txt(d.municipio),'Município');
  exigir(!!txt(d.uf),'UF residencial');
  exigir(!!txt(d.grauFormacao),'Maior Grau de Formação');
  if(avisoIdade)pendencias.push('Cronologia acadêmica exige conferência');

  const cursos=d.cursosSuperiores.filter(c=>txt(c.tipo)||txt(c.area)||txt(c.curso)||txt(c.instituicao)||txt(c.ano));
  if(norm(d.grauFormacao)==='ENSINO SUPERIOR'){
    if(!cursos.length)pendencias.push('Graduação não informada');
    cursos.forEach((c,i)=>{
      if(!txt(c.tipo)||!txt(c.area)||!txt(c.curso)||!txt(c.instituicao)||!/^\d{4}$/.test(txt(c.ano)))pendencias.push(`Graduação ${i+1} incompleta`);
      else if(!anoConclusaoPlausivel(c.ano,d.dataNasc))pendencias.push(`Ano da graduação ${i+1} incompatível com a data de nascimento`);
    });
  }

  const pos=d.posGraduacoes.filter(p=>txt(p.tipo)||txt(p.area)||txt(p.ano));
  if(d.possuiPos===null)pendencias.push('Situação da pós-graduação sem fonte segura');
  if(d.possuiPos===false&&pos.length)pendencias.push('Pós-graduação contraditória');
  if(d.possuiPos===true){
    if(!pos.length)pendencias.push('Pós-graduação indicada sem detalhes');
    pos.forEach((p,i)=>{if(!txt(p.tipo)||!txt(p.area)||!/^\d{4}$/.test(txt(p.ano)))pendencias.push(`Pós-graduação ${i+1} incompleta`)});
    const anosGrad=cursos.map(c=>Number(c.ano)).filter(Number.isFinite);
    const anoGradMin=anosGrad.length?Math.min(...anosGrad):NaN;
    const anoMestrado=Number(pos.find(p=>norm(p.tipo)==='MESTRADO'&&p.ano)?.ano||NaN);
    pos.forEach((p,i)=>{
      const ano=Number(p.ano);if(!Number.isFinite(ano))return;
      if(Number.isFinite(anoGradMin)&&ano<anoGradMin)pendencias.push(`Pós-graduação ${i+1} anterior à graduação`);
      if(norm(p.tipo)==='DOUTORADO'&&Number.isFinite(anoMestrado)&&ano<anoMestrado)pendencias.push('Doutorado anterior ao mestrado');
    });
  }
  return uniq(pendencias);
}
...
function SearchableSelectField({labelText,value,onChange,options,disabled=false}:{labelText:string;value:string;onChange:(v:string)=>void;options:readonly string[];disabled?:boolean}){
  ...
  const encontradas=termo?options.filter(opcao=>norm(opcao).includes(termo)):options;
  const visiveis=value&&!encontradas.includes(value)?[value,...encontradas]:encontradas;
  const correspondenciaExata=options.some(opcao=>norm(opcao)===termo);
  return<div style={{display:'grid',gap:5,gridColumn:'span 2'}}><span style={{color:theme.textSecondary,fontSize:10.5,fontWeight:800}}>{labelText}</span><input aria-label={`Buscar ${labelText}`} type="search" style={{...input,width:'100%',opacity:disabled?0.65:1}} value={buscaOpcao} disabled={disabled} onChange={e=>setBuscaOpcao(e.target.value)} placeholder="Buscar ou digitar o nome da instituição..."/><select aria-label={labelText} style={{...input,width:'100%',opacity:disabled?0.65:1}} value={value} disabled={disabled} onChange={e=>onChange(e.target.value)}><option value="">{termo?`${encontradas.length} resultado(s) — selecione...`:'Selecione a instituição...'}</option>{visiveis.map(o=><option key={o} value={o}>{o}</option>)}</select>{termo&&!correspondenciaExata&&<button type="button" style={btn('ghost',{small:true})} onClick={()=>onChange(buscaOpcao.trim())}>Usar “{buscaOpcao.trim()}” como nome informado na ficha</button>}{termo&&!encontradas.length&&<span style={{color:theme.warning,fontSize:10.5,fontWeight:750}}>A instituição não consta da lista atual da Secretaria; o nome informado na ficha pode ser usado.</span>}</div>
}
...
export default function EducacensoDocentes(){
  ...
  const[fonteEducacenso,setFonteEducacenso]=useState<FonteEducacenso|null>(null),[refFicha,setRefFicha]=useState<ReferenciaFicha|null>(null),[avisoConsolidacao,setAvisoConsolidacao]=useState(''),[enviando,setEnviando]=useState(false);
  ...
  const[envioLote,setEnvioLote]=useState(false),[resultadoLote,setResultadoLote]=useState<Array<{rf:string;nome:string;status:'enviado'|'ja_enviado'|'pendente'|'erro';detalhe:string}>>([]);

  const bridge=async(body:Record<string,any>,explicitToken?:string)=>{const t=explicitToken??token,headers:Record<string,string>={};if(t)headers['x-censo-token']=t;const functionName=body.action==='submit'?'censo-submit-guard':'censo-oficial-bridge';const{data,error}=await supabase.functions.invoke(functionName,{body,headers});if(error){let message=error.message||'Falha na integração direta.';const ctx=(error as any).context;try{if(ctx&&typeof ctx.json==='function'){const detail=await ctx.json();if(detail?.erro)message=detail.erro}}catch{}throw new Error(message)}if(!data?.ok)throw new Error(data?.erro||'Operação não autorizada.');return data};
  ...
  const totalEnviados=useMemo(()=>linhas.filter(l=>(enviosPorRf.get(dig(l.servidor.rf))||[]).some(e=>e.status==='enviado')).length,[linhas,enviosPorRf]);
  ...
  const areasGrad=(c:CursoSuperior)=>uniq(opcoes.graduacao.filter(x=>!c.tipo||txt(x.tipo)===c.tipo).map(x=>x.area));
  const cursosGrad=(c:CursoSuperior)=>uniq(opcoes.graduacao.filter(x=>(!c.tipo||txt(x.tipo)===c.tipo)&&(!c.area||txt(x.area)===c.area)).map(x=>x.curso));
  const categoriasInst=(c:CursoSuperior)=>uniq(opcoes.instituicoes.filter(x=>!c.ufInstituicao||txt(x.uf)===c.ufInstituicao).map(x=>x.categoria||x.org));
  const instituicoesInst=(c:CursoSuperior)=>uniq(opcoes.instituicoes.filter(x=>(!c.ufInstituicao||txt(x.uf)===c.ufInstituicao)&&(!c.categoriaOrg||txt(x.categoria||x.org)===c.categoriaOrg)).map(x=>x.nome));
  ...
  const prepararDireto=async(l:Linha)=>{if(!sessaoOk||!token){setAviso('Desbloqueie primeiro a integração direta segura com sua senha do Diário.');document.getElementById('censo-seguranca')?.scrollIntoView({behavior:'smooth',block:'center'});return}if(!competencia){setAviso('Não há competência de frequência disponível para validar este vínculo.');return}setPreparandoRf(l.servidor.rf);setAviso('');setAvisoConsolidacao('');setResultado(null);setFonteEducacenso(null);setRefFicha(null);try{const data=await bridge({action:'prepare',rf:dig(l.servidor.rf),competencia}) as PrepareResponse;let base:EducacensoLookup|null=null;const temMestre=!!data.local?.mestre?.dados;const cpf=dig(data.local?.mestre?.dados?.CPF||data.official?.cpf||data.local?.ficha?.dados?.cpf||data.local?.profile?.cpf);if(cpf.length===11){try{base=await educacensoLookup(cpf);setAvisoConsolidacao(base.avisoConsolidacao||(!base.registro?'CPF não localizado na fotografia atual do Educacenso.':''))}catch(e:any){setAviso(`Cadastro preparado, mas a fotografia do Educacenso não pôde ser consultada: ${e?.message||'erro de consulta'}.`)}}setLinhaEmEdicao(l);setOpcoes(data.opcoesOficiais||EMPTY_OPTIONS);const construido=construirDraft(l,data,base);setDraft(construido.draft);if(construido.avisoIdade)setAvisoConsolidacao(a=>a?`${a} ${construido.avisoIdade}`:construido.avisoIdade);setFonteMatch(temMestre?'Banco Mestre Censo Docentes 2026 + complemento Educacenso':data.local?.profileMatch||'ficha consolidada');setRefFicha(temMestre?null:referenciaFicha(data));if(base?.registro)setFonteEducacenso({referenciaData:base.registro.referencia_data||base.referenciaData||'',arquivoNome:base.registro.arquivo_nome||base.arquivoNome||'',match:base.match||'cpf'});setTimeout(()=>document.getElementById('censo-direto-editor')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}catch(e:any){const msg=e?.message||'Não foi possível preparar este vínculo.';setAviso(msg);if(norm(msg).includes('JA POSSUI UM CADASTRO')||norm(msg).includes('CADASTRO REALIZADO'))await carregarEnvios()}finally{setPreparandoRf('')}};

  const enviarLoteAuditado=async()=>{
    ...
    if(!competencia){setAviso('Não há competência de frequência disponível para auditar os vínculos.');return}
    setEnvioLote(true);setResultadoLote([]);setAviso('');
    const alvos=linhas.filter(l=>!(enviosPorRf.get(dig(l.servidor.rf))||[]).some(e=>e.status==='enviado'||e.status==='confirmado_secretaria'||e.status==='revisao_necessaria'));
    const registrar=(item:{rf:string;nome:string;status:'enviado'|'ja_enviado'|'pendente'|'erro';detalhe:string})=>setResultadoLote(atual=>[...atual,item]);
    let enviadosAgora=0,jaEnviados=0,pendentesAgora=0,errosAgora=0;
    try{
      for(const l of alvos){
        const rf=dig(l.servidor.rf),nome=l.servidor.nome;
        try{
          const data=await bridge({action:'prepare',rf,competencia}) as PrepareResponse;
          let base:EducacensoLookup|null=null;
          const cpf=dig(data.local?.mestre?.dados?.CPF||data.official?.cpf||data.local?.ficha?.dados?.cpf||data.local?.profile?.cpf);
          if(cpf.length===11){try{base=await educacensoLookup(cpf)}catch{}}
          const construido=construirDraft(l,data,base);
          const pronto:Draft={
            ...construido.draft,
            nome:first(data.official?.nome,construido.draft.nome),
            cpf:dig(first(data.official?.cpf,construido.draft.cpf)),
            dataNasc:dataIso(first(data.official?.dataNascimento,construido.draft.dataNasc)),
            deficiencias:['Não possuo deficiência'],
          };
          const pendencias=auditarDraftParaLote(pronto,construido.avisoIdade);
          if(pendencias.length){
            pendentesAgora++;registrar({rf,nome,status:'pendente',detalhe:pendencias.join(' · ')});continue;
          }
          try{
            await bridge({action:'submit',dados:pronto,competencia,requestId:crypto.randomUUID()});
            enviadosAgora++;registrar({rf,nome,status:'enviado',detalhe:'Conferido e enviado ao formulário oficial.'});
          }catch(e:any){
            errosAgora++;registrar({rf,nome,status:'erro',detalhe:`${e?.message||'Falha no envio.'} O sistema não repetirá este envio automaticamente.`});
          }
        }catch(e:any){
          const msg=String(e?.message||'Não foi possível preparar este vínculo.');
          const n=norm(msg);
          if(n.includes('JA POSSUI UM CADASTRO')||n.includes('CADASTRO REALIZADO')){
            pendentesAgora++;registrar({rf,nome,status:'pendente',detalhe:'A Secretaria informou que este RF já possui cadastro, mas o conteúdo não foi validado. Não foi contabilizado como enviado correto.'});
          }else{
            errosAgora++;registrar({rf,nome,status:'erro',detalhe:msg});
          }
        }
      }
      await carregarEnvios();
      setAviso(`Auditoria em lote concluída: ${enviadosAgora} enviado(s), ${jaEnviados} já cadastrado(s), ${pendentesAgora} retido(s) para conferência e ${errosAgora} erro(s). Nenhum cadastro com pendência foi enviado.`);
    }finally{setEnvioLote(false)}
  };
  ...
  const enviarDireto=async()=>{if(!draft||!linhaEmEdicao||!competencia)return;setEnviando(true);setAviso('');setResultado(null);try{const data=await bridge({action:'submit',dados:draft,competencia,requestId:crypto.randomUUID()});setResultado(data.result??data);await carregarEnvios();setAviso('Envio concluído pelo canal direto. O retorno oficial foi registrado para auditoria.')}catch(e:any){setAviso(e?.message||'Falha no envio. Nada deve ser reenviado sem conferir o aviso.')}finally{setEnviando(false)}};
  ...
  return<div style={{marginTop:4}}>
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:20,marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div><h1 style={{margin:0,color:theme.text,fontSize:24}}>👩‍🏫 Censo Docentes 2026</h1><p style={{color:theme.textSecondary,margin:'7px 0 0',maxWidth:900,lineHeight:1.55}}>Formulário oficial com preenchimento automático pelo Banco Mestre Censo Docentes 2026, importado da planilha limpa e localizado pelo RF.</p></div><span style={{color:sessaoOk?theme.success:theme.warning,fontWeight:850,fontSize:11}}>{sessaoOk?'🔐 Integração direta segura ativa':'🔒 Integração direta bloqueada'}</span></div>
    ...
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:14,marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}>
        <div><div style={{color:theme.text,fontWeight:900,fontSize:14}}>⚡ Auditoria e envio em lote</div><div style={{color:theme.textSecondary,fontSize:11,marginTop:4}}>Faz sozinho: Preparar direto → cruzar Banco Mestre/Educacenso/Secretaria → marcar “Não possuo deficiência” → auditar → enviar somente os vínculos sem pendência.</div></div>
        <button style={btn('success')} disabled={envioLote} onClick={()=>void enviarLoteAuditado()}>{envioLote?`Auditando e enviando... ${resultadoLote.length}/${linhas.filter(l=>!enviosPorRf.has(dig(l.servidor.rf))).length}`:'Auditar e enviar todos os docentes prontos'}</button>
      </div>
      {resultadoLote.length>0&&<details style={{marginTop:10}} open={envioLote}><summary style={{cursor:'pointer',color:theme.textSecondary,fontSize:11,fontWeight:850}}>Resultado da execução ({resultadoLote.length})</summary><div style={{display:'grid',gap:5,marginTop:8}}>{resultadoLote.map((r,i)=><div key={`${r.rf}-${i}`} style={{padding:'7px 9px',borderRadius:7,border:`1px solid ${r.status==='enviado'||r.status==='ja_enviado'?theme.success:r.status==='pendente'?theme.warning:theme.danger}55`,fontSize:10.5,color:theme.text}}><strong>{r.status==='enviado'?'✅ ENVIADO':r.status==='ja_enviado'?'✓ JÁ CADASTRADO':r.status==='pendente'?'⚠ RETIDO':'❌ ERRO'} — {r.nome} · RF {formatRf(r.rf)}</strong><div style={{marginTop:3,color:theme.textSecondary}}>{r.detalhe}</div></div>)}</div></details>}
    </section>
    ...
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1000}}><thead><tr style={{background:'var(--ghost-bg)'}}>{['Professor','RF','Frequência','Período','Turma/atuação','Modalidade sugerida','Ação'].map(h=><th key={h} style={{padding:'11px 8px',textAlign:'left',color:theme.textSecondary,fontSize:10.5,borderBottom:`1px solid ${theme.border}`}}>{h}</th>)}</tr></thead><tbody>{filtradas.map((l,idx)=>{const enviosLinha=enviosDaLinha(l),envio=enviosLinha[0]||null;const enviadoOk=envio?.status==='enviado',revisao=envio?.status==='revisao_necessaria',existente=envio?.status==='confirmado_secretaria';const corStatus=enviadoOk?theme.success:revisao?theme.danger:existente?theme.warning:theme.borderLight;return <tr key={`${l.servidor.rf}-${l.servidor.nome}`} style={{borderBottom:`1px solid ${corStatus}`,background:envio?`${corStatus}12`:(idx%2===0?'var(--row-even)':'var(--row-odd)')}}><td style={{padding:'10px 8px',fontSize:12,color:theme.text}}><strong>{l.servidor.nome}</strong>{envio&&<div style={{color:corStatus,fontSize:10,fontWeight:900,marginTop:3}}>{enviadoOk?'✅ ENVIADO':revisao?'⚠️ REVISÃO NECESSÁRIA':'⚠️ CADASTRO EXISTE NA SECRETARIA — CONTEÚDO NÃO VALIDADO'} · {formatDataHora(envio.criadoEm)}{envio.modalidade?` · ${envio.modalidade}`:''}</div>}<div style={{color:theme.textMuted,fontSize:9.5,marginTop:3}}>{l.frequencia.cargo||l.servidor.cargo}</div>{l.vinculosMesmoNome>1&&<div style={{color:theme.warning,fontSize:9.5,fontWeight:800}}>⚠️ {l.vinculosMesmoNome} RFs — tratar por vínculo</div>}</td><td style={{padding:'10px 8px',fontWeight:850,color:theme.text}}>{formatRf(l.servidor.rf)}</td><td style={{padding:'10px 8px',color:theme.success,fontSize:11}}>✓ pág. {l.frequencia.pagina_pdf??'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.servidor.periodo||'—'}</td><td style={{padding:'10px 8px',color:theme.textSecondary,fontSize:11}}>{l.turmas.length?l.turmas.map(t=>t.nome).join(', '):'⚠️ conferir'}</td><td style={{padding:'10px 8px',color:l.modalidade?theme.textSecondary:theme.warning,fontSize:11}}>{l.modalidade||'conferir'}</td><td style={{padding:'10px 8px'}}>{envio?<div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}><span style={{padding:'6px 9px',borderRadius:999,background:`${corStatus}18`,border:`1px solid ${corStatus}55`,color:corStatus,fontWeight:900,fontSize:10.5}}>{enviadoOk?'✅ ENVIADO':revisao?'⚠️ REVISÃO NECESSÁRIA':'⚠️ JÁ CADASTRADO — NÃO VALIDADO'}</span>{revisao&&<span style={{color:theme.danger,fontSize:10,fontWeight:800}}>{envio.motivoRevisao||'Conteúdo informado como incorreto.'}</span>}{existente&&<span title={envio.confirmacaoMensagem||'A Secretaria informa apenas que o cadastro existe.'} style={{color:theme.warning,fontSize:10}}>A existência foi confirmada; o conteúdo não foi validado.</span>}{enviosLinha.filter(e=>e.status==='enviado'&&e.comprovanteUrl).map((e,i)=><a key={`${e.modalidade}-${e.criadoEm}`} href={e.comprovanteUrl} target="_blank" rel="noopener noreferrer" style={{...btn('ghost',{small:true}),textDecoration:'none'}}>📄 Comprovante {enviosLinha.length>1?i+1:''}</a>)}<button style={btn('ghost',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>Revisar</button></div>:<button style={btn('primary',{small:true})} disabled={preparandoRf===l.servidor.rf} onClick={()=>prepararDireto(l)}>{preparandoRf===l.servidor.rf?'Preparando...':'Preparar direto'}</button>}</td></tr>})}</tbody></table></div></section>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      ...
      <div style={{marginTop:11,padding:10,borderRadius:8,background:`${theme.success}0F`,border:`1px solid ${theme.success}55`,color:theme.textSecondary,fontSize:11.5,lineHeight:1.5}}><strong style={{color:theme.success}}>✓ Planilha limpa aplicada:</strong> os dados foram carregados do Banco Mestre Censo Docentes 2026 salvo definitivamente no banco. Qualquer campo pode ser ajustado e nenhum aviso de preenchimento bloqueia o envio.</div>
      ...
      <div style={{marginTop:12,color:theme.textSecondary,fontSize:11.5,fontWeight:850}}>Curso(s) Superior(es) — máximo 3</div><div style={{display:'grid',gap:8,marginTop:7}}>{draft.cursosSuperiores.map((c,i)=><div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:8,padding:10,background:'var(--ghost-bg)'}}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:7}}><SelectField labelText="1º Tipo" value={c.tipo} onChange={v=>atualizarCurso(i,{tipo:v,area:'',curso:''})} options={gradTipos}/><SelectField labelText="2º Área" value={c.area} onChange={v=>atualizarCurso(i,{area:v,curso:''})} options={areasGrad(c)}/><SelectField labelText="3º Curso" value={c.curso} onChange={v=>atualizarCurso(i,{curso:v})} options={cursosGrad(c)}/><Field labelText="Ano Conclusão" value={c.ano} onChange={v=>atualizarCurso(i,{ano:v})}/><SelectField labelText="4º UF Inst." value={c.ufInstituicao} onChange={v=>atualizarCurso(i,{ufInstituicao:v,categoriaOrg:'',instituicao:''})} options={instUfs}/><SelectField labelText="5º Categoria / Org." value={c.categoriaOrg} onChange={v=>atualizarCurso(i,{categoriaOrg:v,instituicao:''})} options={categoriasInst(c)}/><SearchableSelectField key={`${i}-${c.ufInstituicao}-${c.categoriaOrg}`} labelText="6º Nome da Instituição" value={c.instituicao} onChange={v=>atualizarCurso(i,{instituicao:v})} options={instituicoesInst(c)}/></div><label style={{display:'flex',gap:7,alignItems:'center',color:theme.textSecondary,fontSize:11,marginTop:8}}><input type="checkbox" checked={c.entregue} onChange={e=>atualizarCurso(i,{entregue:e.target.checked})}/>Cópia entregue nesta UE</label><button style={{...btn('ghost',{small:true}),marginTop:7}} onClick={()=>patch('cursosSuperiores',draft.cursosSuperiores.filter((_,idx)=>idx!==i))}>Remover curso</button></div>)}{draft.cursosSuperiores.length<3&&<button style={btn('ghost',{small:true})} onClick={()=>patch('cursosSuperiores',[...draft.cursosSuperiores,cursoSuperiorVazio()])}>+ Adicionar Curso Superior</button>}</div>
      ...
      <div style={{marginTop:15,padding:12,borderRadius:8,border:`1px solid ${theme.success}55`,background:`${theme.success}0F`}}><div style={{color:theme.success,fontSize:11.5,fontWeight:850}}>✓ Dados copiados das fontes disponíveis, sem validação de correspondência ou completude.</div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}><button style={btn('success')} disabled={enviando} onClick={enviarDireto}>{enviando?'Enviando...':'✅ Salvar e enviar ao formulário oficial'}</button><a href={FORM_URL} target="_blank" rel="noopener noreferrer" style={{...btn('ghost'),textDecoration:'none'}}>Abrir formulário manual</a></div></div>
