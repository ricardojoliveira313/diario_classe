...
export default function EducacensoDocentes(){
  ...
  const[resultado,setResultado]=useState<any>(null),[opcoes,setOpcoes]=useState<OpcoesOficiais>(EMPTY_OPTIONS);
  ...
  const bloquear=async()=>{try{if(token)await bridge({action:'logout'})}catch{}sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false);setDraft(null);setLinhaEmEdicao(null);setFonteEducacenso(null);setRefFicha(null);setAviso('Sessão segura do Censo encerrada.')};
