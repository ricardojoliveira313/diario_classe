...
function nomesCompletosIguais(a:string,b:string){const na=norm(a),nb=norm(b);return!!na&&!!nb&&na===nb}
function turmasDoServidor(s:Servidor,f:Frequencia,turmas:Turma[]){const atribuida=norm(s.turma_atribuida||s.sala_atribuida||'');if(atribuida){const x=turmas.filter(t=>norm(t.nome)===atribuida);if(x.length)return x}return turmas.filter(t=>nomesCompletosIguais(txt(t.professora),txt(f.nome)))}
...
export default function EducacensoDocentes(){
  ...
  const linhas=useMemo<Linha[]>(()=>docentesFreq.map(freq=>{const servidor=servidorPorRf.get(dig(freq.rf))??{rf:freq.rf,nome:freq.nome,cargo:freq.cargo||'',periodo:'',escola:freq.local_trabalho||SCHOOL,ativo:true,turma_atribuida:'',sala_atribuida:''};const ts=turmasDoServidor(servidor,freq,turmas);return{servidor,frequencia:freq,turmas:ts,modalidade:modalidadeSugerida(servidor,freq,ts),vinculosMesmoNome:nomesDuplicados.get(norm(freq.nome))??1}}),[docentesFreq,servidorPorRf,turmas,nomesDuplicados]);
