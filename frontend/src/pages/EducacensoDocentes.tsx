...
type EnvioStatus = { rf:string; modalidade:string; status:'enviado'; criadoEm:string; comprovanteUrl:string; confirmadoSecretaria?:boolean; confirmacaoMensagem?:string };
...
export default function EducacensoDocentes(){
  ...
  const enviarLoteAuditado=async()=>{
    ...
    const alvos=linhas.filter(l=>!enviosPorRf.has(dig(l.servidor.rf)));
      ...
      for(const l of alvos){
        ...
        try{
          ...
          if(n.includes('JA POSSUI UM CADASTRO')||n.includes('CADASTRO REALIZADO')){
            jaEnviados++;registrar({rf,nome,status:'ja_enviado',detalhe:'A Secretaria informou que este RF já possui cadastro realizado.'});
          }else{
