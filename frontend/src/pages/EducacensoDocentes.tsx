...
function cursosMestre(dados:Record<string,any>,opcoes:OpcoesOficiais):CursoSuperior[]{
  ...
  for(let i=1;i<=3;i++){
    ...
    const item:CursoSuperior={
      ...
      categoriaOrg:first(dados[`${i}º Categoria/Org.`],instituicao?.categoriaOrg),instituicao:instituicao?.instituicao||instituicaoRaw,
      entregue:simPlanilha(dados[`${i}º Cópia entregue UE`]),
    };
    if(Object.entries(item).some(([chave,valor])=>chave!=='entregue'&&txt(valor)))cursos.push(item);
  }
  return cursos;
}
...
function construirDraft(l:Linha,r:PrepareResponse,base:EducacensoLookup|null):{draft:Draft;avisoIdade:string}{
  ...
  const possuiPos:boolean|null=temMestre?(temPosEstruturada?true:possuiPosPlanilha):temPosEstruturada?true:temIndicacaoPosEducacenso?true:possuiPosFicha!==null?possuiPosFicha:posRawEdu==='-'?false:null;
  const posGraduacoesBrutas=temMestre?posEstruturada:temPosEstruturada?posEstruturada:temPosEducacenso?posEdu:possuiPos===true?[posVazia()]:[];
  const superioresCheck=temMestre?{itens:superioresBrutos,descartados:0}:limparAnosImplausiveis(superioresBrutos,dataNasc);
  const posCheckIdade=temMestre?{itens:posGraduacoesBrutas,descartados:0}:limparAnosImplausiveis(posGraduacoesBrutas,dataNasc);
  const posCheckCronologia=temMestre?{pos:posCheckIdade.itens,descartados:0}:limparCronologiaPos(superioresCheck.itens,posCheckIdade.itens);
  ...
  const draft:Draft={
    ...
    dataNasc,sexo:temMestre?inOptions(m.Sexo,SEXOS):sexoEdu,
    corRaca:temMestre?mapCorRaca(m['Cor/Raça']):(corEdu||CORES_RACAS.find(c=>norm(c)===norm(etnia))||''),telefone:first(temMestre?m['Telefone/Celular']:'',f.telefone1,p.telefone_celular_1,f.telefone2,p.telefone_celular_2,p.telefone_fixo),
    email:first(temMestre?m['E-mail']:'',f.email,p.email),nomeMae:first(temMestre?m['Filiação 1']:'',f.nomeMae,p.nome_mae),nomePai:first(temMestre?m['Filiação 2']:'',f.nomePai,p.nome_pai),nacionalidade:temMestre?(inOptions(m.Nacionalidade,NACIONALIDADES)||txt(m.Nacionalidade)):nacionalidadeEdu||'Brasileiro(a)',paisEstrangeiro:'',
    ...
    deficiencias:temMestre?itensPlanilha(m['Deficiência/TEA/Altas Habilidades'],DEFICIENCIAS):parseDeficienciasEducacenso(educacenso?.deficiencia_raw),grauFormacao:temMestre?(inOptions(m['Maior Grau de Formação'],GRAUS)||txt(m['Maior Grau de Formação'])):grauEdu||grauLocal,cursosSuperiores:superiores.length?superiores:[cursoSuperiorVazio()],possuiPos,
    posGraduacoes:possuiPos?(posGraduacoes.length?posGraduacoes:[posVazia()]):[],cursosEspecificos:temMestre?itensPlanilha(m['Cursos específicos 80h'],CURSOS_ESPECIFICOS):parseCursosEducacenso(educacenso?.cursos_especificos_raw),
  };
...
function SearchableSelectField({labelText,value,onChange,options,disabled=false}:{labelText:string;value:string;onChange:(v:string)=>void;options:readonly string[];disabled?:boolean}){
  ...
  return<div style={{display:'grid',gap:5,gridColumn:'span 2'}}><span style={{color:theme.textSecondary,fontSize:10.5,fontWeight:800}}>{labelText}</span><input aria-label={`Buscar ${labelText}`} type="search" style={{...input,width:'100%',opacity:disabled?0.65:1}} value={buscaOpcao} disabled={disabled} onChange={e=>setBuscaOpcao(e.target.value)} placeholder="Buscar pelo nome da instituição..."/><select aria-label={labelText} style={{...input,width:'100%',opacity:disabled?0.65:1}} value={value} disabled={disabled} onChange={e=>onChange(e.target.value)}><option value="">{termo?`${encontradas.length} resultado(s) — selecione...`:'Selecione a instituição...'}</option>{visiveis.map(o=><option key={o} value={o}>{o}</option>)}</select>{termo&&!encontradas.length&&<span style={{color:theme.warning,fontSize:10.5,fontWeight:750}}>Nenhuma instituição oficial encontrada com esse nome.</span>}</div>
}
...
export default function EducacensoDocentes(){
  ...
  const prepararDireto=async(l:Linha)=>{if(!sessaoOk||!token){setAviso('Desbloqueie primeiro a integração direta segura com sua senha do Diário.');document.getElementById('censo-seguranca')?.scrollIntoView({behavior:'smooth',block:'center'});return}if(!competencia){setAviso('Não há competência de frequência disponível para validar este vínculo.');return}setPreparandoRf(l.servidor.rf);setAviso('');setAvisoConsolidacao('');setResultado(null);setConfirmacao(false);setFonteEducacenso(null);setRefFicha(null);try{const data=await bridge({action:'prepare',rf:dig(l.servidor.rf),competencia}) as PrepareResponse;let base:EducacensoLookup|null=null;const temMestre=!!data.local?.mestre?.dados;if(!temMestre){const cpf=dig(data.official?.cpf||data.local?.ficha?.dados?.cpf||data.local?.profile?.cpf);if(cpf.length===11){try{base=await educacensoLookup(cpf);setAvisoConsolidacao(base.avisoConsolidacao||(!base.registro?'CPF não localizado na fotografia atual do Educacenso.':''))}catch(e:any){setAviso(`Cadastro preparado, mas a fotografia do Educacenso não pôde ser consultada: ${e?.message||'erro de consulta'}.`)}}}setLinhaEmEdicao(l);setOpcoes(data.opcoesOficiais||EMPTY_OPTIONS);const construido=construirDraft(l,data,base);setDraft(construido.draft);if(construido.avisoIdade)setAvisoConsolidacao(a=>a?`${a} ${construido.avisoIdade}`:construido.avisoIdade);setFonteMatch(temMestre?'Banco Mestre Censo Docentes 2026':data.local?.profileMatch||'ficha consolidada');setRefFicha(temMestre?null:referenciaFicha(data));if(base?.registro)setFonteEducacenso({referenciaData:base.registro.referencia_data||base.referenciaData||'',arquivoNome:base.registro.arquivo_nome||base.arquivoNome||'',match:base.match||'cpf'});setTimeout(()=>document.getElementById('censo-direto-editor')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}catch(e:any){setAviso(e?.message||'Não foi possível preparar este vínculo.')}finally{setPreparandoRf('')}};
