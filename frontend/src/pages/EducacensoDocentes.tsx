...
function cursoSuperiorVazio(): CursoSuperior {
  return { tipo: '', area: '', curso: '', ufInstituicao: '', categoriaOrg: '', instituicao: '', entregue: false };
}

function posVazia(): PosGraduacao {
  return { tipo: '', area: '', entregue: false };
}
...
export default function EducacensoDocentes() {
  ...
  const toggleCursoEspecifico = (value: string) => {
    if (!draft) return;
    let next = draft.cursosEspecificos.includes(value) ? draft.cursosEspecificos.filter(x => x !== value) : [...draft.cursosEspecificos, value];
  ...
  const atualizarCurso = (i: number, key: keyof CursoSuperior, value: string | boolean) => {
    if (!draft) return;
    patch('cursosSuperiores', draft.cursosSuperiores.map((c, idx) => idx === i ? ({ ...c, [key]: value }) : c));
  };

  const atualizarPos = (i: number, key: keyof PosGraduacao, value: string | boolean) => {
    if (!draft) return;
    patch('posGraduacoes', draft.posGraduacoes.map((c, idx) => idx === i ? ({ ...c, [key]: value }) : c));
  };
  ...
  const validarDraft = () => {
    ...
    const obrigatorios: Array<[string, string]> = [
      ['Modalidade', draft.modalidade], ['CPF', draft.cpf], ['Data de nascimento', draft.dataNasc],
      ['Sexo', draft.sexo], ['Cor/Raça', draft.corRaca], ['Telefone', draft.telefone],
      ['E-mail', draft.email], ['Nacionalidade', draft.nacionalidade], ['CEP', draft.cep],
      ['Rua', draft.rua], ['Bairro', draft.bairro], ['Município', draft.municipio], ['UF', draft.uf],
      ['Grau de formação', draft.grauFormacao],
    ];
    obrigatorios.forEach(([k, v]) => { if (!txt(v)) faltam.push(k); });
    if (draft.nacionalidade === 'Estrangeiro(a)' && !txt(draft.paisEstrangeiro)) faltam.push('País de nacionalidade');
    ...
    if (draft.grauFormacao === 'Ensino Superior') {
      if (!draft.cursosSuperiores.length) faltam.push('Curso superior');
      draft.cursosSuperiores.forEach((c, i) => {
        const n = i + 1;
        if (!c.tipo) faltam.push(`Tipo do ${n}º curso superior`);
        if (!c.area) faltam.push(`Área do ${n}º curso superior`);
        if (!c.curso) faltam.push(`Curso do ${n}º curso superior`);
        if (!c.ufInstituicao) faltam.push(`UF da instituição do ${n}º curso superior`);
        if (!c.categoriaOrg) faltam.push(`Categoria/Org. do ${n}º curso superior`);
        if (!c.instituicao) faltam.push(`Instituição do ${n}º curso superior`);
      });
    }

    if (draft.possuiPos) {
      if (!draft.posGraduacoes.length) faltam.push('Pós-graduação');
      draft.posGraduacoes.forEach((p, i) => {
        if (!p.tipo) faltam.push(`Tipo da ${i + 1}ª pós-graduação`);
        if (!p.area) faltam.push(`Área da ${i + 1}ª pós-graduação`);
      });
    }
      ...
      <section style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: theme.radiusMd, boxShadow: theme.shadow, padding: 20, marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ color: theme.textSecondary, margin: '7px 0 0', maxWidth: 850, lineHeight: 1.55 }}>Campos alinhados ao formulário oficial da Secretaria: frequência + cadastro funcional + ficha cadastral + conferência humana.</p>
          </div>
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
            <span style={{ color: theme.primaryText, background: 'var(--ghost-bg)', border: `1px solid ${theme.border}`, borderRadius: 999, padding: '6px 10px', fontSize: 11, fontWeight: 850 }}>{SCHOOL}</span>
            <span style={{ color: sessaoOk ? theme.success : theme.warning, background: `${sessaoOk ? theme.success : theme.warning}18`, border: `1px solid ${sessaoOk ? theme.success : theme.warning}55`, borderRadius: 999, padding: '6px 10px', fontSize: 11, fontWeight: 850 }}>{sessaoOk ? '🔐 Integração direta segura ativa' : '🔒 Integração direta bloqueada'}</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 15 }}>
          ...
          <a href={FORM_URL} target="_blank" rel="noopener noreferrer" style={{ ...btn('ghost'), textDecoration: 'none' }}>Fallback: abrir formulário oficial</a>
        </div>
      ...
      <section id="censo-seguranca" style={{ background: theme.card, border: `1px solid ${sessaoOk ? theme.success : theme.warning}55`, borderRadius: theme.radiusMd, boxShadow: theme.shadow, padding: 15, marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ color: theme.textSecondary, fontSize: 11.5, marginTop: 4 }}>Os dados são buscados por RF e o envio exige conferência e confirmação final.</div>
          </div>
          {sessaoOk ? (
      ...
      <section style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: theme.radiusMd, boxShadow: theme.shadow, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1000 }}>
            <thead><tr style={{ background: 'var(--ghost-bg)' }}>{['Professor', 'RF', 'Frequência', 'Período', 'Turma/atuação', 'Modalidade sugerida', 'Ação'].map(h => <th key={h} style={{ padding: '11px 8px', textAlign: 'left', color: theme.textSecondary, fontSize: 10.5, borderBottom: `1px solid ${theme.border}` }}>{h}</th>)}</tr></thead>
            <tbody>{filtradas.map((l, idx) => (
              <tr key={`${l.servidor.rf}-${l.servidor.nome}`} style={{ borderBottom: `1px solid ${theme.borderLight}`, background: idx % 2 === 0 ? 'var(--row-even)' : 'var(--row-odd)' }}>
                <td style={{ padding: '10px 8px', fontSize: 12, color: theme.text }}><strong>{l.servidor.nome}</strong><div style={{ color: theme.textMuted, fontSize: 9.5, marginTop: 3 }}>{l.frequencia.cargo || l.servidor.cargo}</div>{l.vinculosMesmoNome > 1 && <div style={{ color: theme.warning, fontSize: 9.5, marginTop: 3, fontWeight: 800 }}>⚠️ {l.vinculosMesmoNome} RFs na frequência — tratar por vínculo</div>}</td>
                <td style={{ padding: '10px 8px', color: theme.text, fontWeight: 850, fontSize: 12 }}>{formatRf(l.servidor.rf)}</td>
                <td style={{ padding: '10px 8px', color: theme.success, fontSize: 11, fontWeight: 800 }}>✓ pág. {l.frequencia.pagina_pdf ?? '—'}<div style={{ color: theme.textMuted, fontSize: 9.5 }}>{l.frequencia.carga_horaria ?? '—'} h</div></td>
                ...
                <td style={{ padding: '10px 8px', whiteSpace: 'nowrap' }}><button style={btn('primary', { small: true })} disabled={preparandoRf === l.servidor.rf} onClick={() => prepararDireto(l)}>{preparandoRf === l.servidor.rf ? 'Preparando...' : 'Preparar direto'}</button></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      ...
      {draft && linhaEmEdicao && (
        <section id="censo-direto-editor" style={{ marginTop: 16, background: theme.card, border: `2px solid ${theme.primaryText}`, borderRadius: theme.radiusMd, boxShadow: theme.shadow, padding: 17 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            <div><h2 style={{ margin: 0, color: theme.text, fontSize: 19 }}>Conferência oficial — {draft.nome}</h2><div style={{ marginTop: 5, color: theme.textSecondary, fontSize: 11.5 }}>RF <strong>{formatRf(draft.rf)}</strong> · CPF {maskCpf(draft.cpf)} · frequência pág. {linhaEmEdicao.frequencia.pagina_pdf ?? '—'} · fonte local: {fonteMatch || 'consolidada'}</div></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 6 }}>
            {DEFICIENCIAS.map(d => <label key={d} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 8px', borderRadius: 7, border: `1px solid ${theme.border}`, color: theme.textSecondary, fontSize: 11.5, cursor: 'pointer' }}><input type="checkbox" checked={draft.deficiencias.includes(d)} onChange={() => toggleDef(d)} />{d}</label>)}
          </div>
          ...
          {draft.grauFormacao === 'Ensino Superior' && (
            <>
              ...
              <div style={{ display: 'grid', gap: 8, marginTop: 7 }}>
                {draft.cursosSuperiores.map((c, i) => <div key={i} style={{ border: `1px solid ${theme.border}`, borderRadius: 8, padding: 10, background: 'var(--ghost-bg)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 7 }}>
                    <SelectField labelText="1º Tipo *" value={c.tipo} onChange={v => atualizarCurso(i, 'tipo', v)} options={TIPOS_SUPERIOR} />
                    <Field labelText="2º Área *" value={c.area} onChange={v => atualizarCurso(i, 'area', v)} />
                    <Field labelText="3º Curso *" value={c.curso} onChange={v => atualizarCurso(i, 'curso', v)} />
                    <SelectField labelText="4º UF Inst. *" value={c.ufInstituicao} onChange={v => atualizarCurso(i, 'ufInstituicao', v)} options={UFS} />
                    <Field labelText="5º Categoria / Org. *" value={c.categoriaOrg} onChange={v => atualizarCurso(i, 'categoriaOrg', v)} />
                    <Field labelText="6º Nome da Instituição *" value={c.instituicao} onChange={v => atualizarCurso(i, 'instituicao', v)} />
                  </div>
                  <label style={{ display: 'flex', gap: 7, alignItems: 'center', color: theme.textSecondary, fontSize: 11, marginTop: 8 }}><input type="checkbox" checked={c.entregue} onChange={e => atualizarCurso(i, 'entregue', e.target.checked)} />Cópia entregue nesta UE</label>
          ...
          {draft.possuiPos && <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
            {draft.posGraduacoes.map((c, i) => <div key={i} style={{ border: `1px solid ${theme.border}`, borderRadius: 8, padding: 10, background: 'var(--ghost-bg)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 7 }}>
                ...
                <SelectField labelText="Área *" value={c.area} onChange={v => atualizarPos(i, 'area', v)} options={AREAS_POS} />
              </div>
              <label style={{ display: 'flex', gap: 7, alignItems: 'center', color: theme.textSecondary, fontSize: 11, marginTop: 8 }}><input type="checkbox" checked={c.entregue} onChange={e => atualizarPos(i, 'entregue', e.target.checked)} />Cópia entregue nesta UE</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 6, marginTop: 7 }}>
            {CURSOS_ESPECIFICOS.map(c => <label key={c} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 8px', borderRadius: 7, border: `1px solid ${theme.border}`, color: theme.textSecondary, fontSize: 11.5, cursor: 'pointer' }}><input type="checkbox" checked={draft.cursosEspecificos.includes(c)} onChange={() => toggleCursoEspecifico(c)} />{c}</label>)}
          </div>
          ...
          <div style={{ marginTop: 15, padding: 12, borderRadius: 8, border: `1px solid ${faltamDraft.length ? theme.warning : theme.success}55`, background: `${faltamDraft.length ? theme.warning : theme.success}0F` }}>
            <div style={{ color: faltamDraft.length ? theme.warning : theme.success, fontSize: 11.5, fontWeight: 850 }}>{faltamDraft.length ? `Pendências antes do envio: ${faltamDraft.join(' · ')}` : '✓ Campos obrigatórios conferidos.'}</div>
            <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', color: theme.text, fontSize: 12, marginTop: 10, lineHeight: 1.45, cursor: 'pointer' }}><input type="checkbox" checked={confirmacao} onChange={e => setConfirmacao(e.target.checked)} style={{ marginTop: 2 }} /><span><strong>Declaro que as informações apresentadas neste formulário são autodeclaradas e assumo total responsabilidade pela veracidade dos dados fornecidos.</strong></span></label>
