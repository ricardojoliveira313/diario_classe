...
function cursosMestre(dados:Record<string,any>,opcoes:OpcoesOficiais):CursoSuperior[]{
  ...
  for(let i=1;i<=3;i++){
    ...
    const estruturada=resolveGraduacaoEstruturada({tipo_oficial:tipo,area_oficial:area,curso_oficial:cursoRaw},opcoes);
    const instituicao=resolveInstituicao(instituicaoRaw,opcoes);
    const categoriaRaw=txt(dados[`${i}º Categoria/Org.`]);
    const categoriaOficial=uniq(opcoes.instituicoes.map(x=>x.categoria||x.org)).find(x=>norm(x)===norm(categoriaRaw))||categoriaRaw;
    const item:CursoSuperior={
      ...cursoSuperiorVazio(),tipo:estruturada?.tipo||tipo,area:estruturada?.area||area,curso:estruturada?.curso||cursoRaw,
      ano:txt(dados[`${i}º Ano Conclusão`]),ufInstituicao:ufSigla(dados[`${i}º UF Instituição`])||instituicao?.ufInstituicao||'',
      categoriaOrg:first(categoriaOficial,instituicao?.categoriaOrg),instituicao:instituicao?.instituicao||instituicaoRaw,
...
function construirDraft(l:Linha,r:PrepareResponse,base:EducacensoLookup|null):{draft:Draft;avisoIdade:string}{
  ...
  const draft:Draft={
    ...
    dataNasc,sexo:inOptions(first(temMestre?m.Sexo:'',educacenso?.sexo),SEXOS),
    corRaca:mapCorRaca(first(temMestre?m['Cor/Raça']:'',educacenso?.cor_raca_oficial))||CORES_RACAS.find(c=>norm(c)===norm(etnia))||'',telefone:first(temMestre?m['Telefone/Celular']:'',f.telefone1,p.telefone_celular_1,f.telefone2,p.telefone_celular_2,p.telefone_fixo),
    email:first(temMestre?m['E-mail']:'',f.email,p.email),nomeMae:first(temMestre?m['Filiação 1']:'',f.nomeMae,p.nome_mae),nomePai:first(temMestre?m['Filiação 2']:'',f.nomePai,p.nome_pai),nacionalidade:inOptions(first(temMestre?m.Nacionalidade:'',educacenso?.nacionalidade_oficial),NACIONALIDADES)||first(temMestre?m.Nacionalidade:'',nacionalidadeEdu,'Brasileiro(a)'),paisEstrangeiro:'',
...
export default function EducacensoDocentes(){
  ...
  return<div style={{marginTop:4}}>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      ...
      <div style={{marginTop:15,padding:12,borderRadius:8,border:`1px solid ${faltamDraft.length?theme.warning:theme.success}55`,background:`${faltamDraft.length?theme.warning:theme.success}0F`}}><div style={{color:faltamDraft.length?theme.warning:theme.success,fontSize:11.5,fontWeight:850}}>{faltamDraft.length?`Avisos de preenchimento — não bloqueiam o envio: ${faltamDraft.join(' · ')}`:'✓ Campos do formulário conferidos.'}</div><label style={{display:'flex',gap:8,alignItems:'flex-start',color:theme.text,fontSize:12,marginTop:10,lineHeight:1.45,cursor:'pointer'}}><input type="checkbox" checked={confirmacao} onChange={e=>setConfirmacao(e.target.checked)} style={{marginTop:2}}/><span><strong>Declaro que as informações apresentadas neste formulário são autodeclaradas e assumo total responsabilidade pela veracidade dos dados fornecidos.</strong></span></label><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}><button style={btn('success')} disabled={enviando||!confirmacao} onClick={enviarDireto}>{enviando?'Enviando...':'✅ Salvar e enviar ao formulário oficial'}</button><a href={FORM_URL} target="_blank" rel="noopener noreferrer" style={{...btn('ghost'),textDecoration:'none'}}>Abrir formulário manual</a></div></div>
