...
const CURSOS_ESPECIFICOS = [
  ...
  'Educação bilíngue de surdos', 'Educação e TIC', 'Educação integral em tempo integral',
  'Gênero e diversidade sexual', 'Direitos da criança e do adolescente', 'Ed. Relações Étnico-Raciais',
...
type CursoSuperior = { tipo: string; area: string; curso: string; ufInstituicao: string; categoriaOrg: string; instituicao: string; entregue: boolean };
type PosGraduacao = { tipo: string; area: string; entregue: boolean };
type Draft = {
...
function ufSigla(value: string) {
  const n = norm(value); const map: Record<string,string> = { ACRE:'AC',ALAGOAS:'AL',AMAPA:'AP',AMAZONAS:'AM',BAHIA:'BA',CEARA:'CE','DISTRITO FEDERAL':'DF','ESPIRITO SANTO':'ES',GOIAS:'GO',MARANHAO:'MA','MATO GROSSO':'MT','MATO GROSSO DO SUL':'MS','MINAS GERAIS':'MG',PARA:'PA',PARAIBA:'PB',PARANA:'PR',PERNAMBUCO:'PE',PIAUI:'PI','RIO DE JANEIRO':'RJ','RIO GRANDE DO NORTE':'RN','RIO GRANDE DO SUL':'RS',RONDONIA:'RO',RORAIMA:'RR','SANTA CATARINA':'SC','SAO PAULO':'SP',SERGIPE:'SE',TOCANTINS:'TO' };
  const sigla = /^[A-Z]{2}$/.test(n) ? n : (map[n] || ''); return UFS.includes(sigla as any) ? sigla : '';
}
function cursoSuperiorVazio(): CursoSuperior { return { tipo:'', area:'', curso:'', ufInstituicao:'', categoriaOrg:'', instituicao:'', entregue:false }; }
function posVazia(): PosGraduacao { return { tipo:'', area:'', entregue:false }; }
...
function construirDraft(l: Linha, r: PrepareResponse): Draft {
  ...
  const superiores: CursoSuperior[] = formacoes.filter((x:any) => !norm(x.tipo).includes('POS')).slice(0,3).map((x:any) => ({
    ...cursoSuperiorVazio(), tipo: inOptions(x.tipo, TIPOS_SUPERIOR), curso: txt(x.curso), instituicao: txt(x.universidade),
  }));
  if (!superiores.length && txt(f.formacaoPrincipal)) superiores.push({ ...cursoSuperiorVazio(), curso: txt(f.formacaoPrincipal), instituicao: universidades[0] || '' });
  const pos: PosGraduacao[] = posCursos.slice(0,6).map((curso:string) => ({ ...posVazia(), area: inOptions(curso, AREAS_POS) }));
  ...
  return {
    ...
    dataNasc: first(official.dataNascimento, f.dataNascimento, p.data_nascimento), sexo: '',
    corRaca: CORES_RACAS.find(c => norm(c) === norm(etnia)) || '', telefone: first(f.telefone1,p.telefone_celular_1,f.telefone2,p.telefone_celular_2,p.telefone_fixo),
    email: first(f.email,p.email), nomeMae: first(f.nomeMae,p.nome_mae), nomePai: first(f.nomePai,p.nome_pai), nacionalidade:'', paisEstrangeiro:'',
    naturalidade: first(f.municipioNascimento,p.municipio_nascimento), ufNascimento: ufSigla(first(f.ufNascimento,p.estado_nascimento)),
    cep: dig(first(f.cep,p.cep)), rua: endereco.rua, numero: endereco.numero, bairro: first(f.bairro,p.bairro), municipio: first(f.municipio,p.municipio), uf: ufSigla(first(f.estado,p.estado)),
    deficiencias: [], grauFormacao:'', cursosSuperiores: superiores,
    possuiPos: norm(f.posPossui) === 'SIM' || pos.length > 0 || formacoes.some((x:any) => norm(x.tipo).includes('POS')),
    posGraduacoes: pos, cursosEspecificos: [],
  };
}
...
function gerarCsv(linhas: Linha[]) {
  ...
  const blob = new Blob(['\ufeff'+[cab.map(esc).join(';'),...corpo].join('\n')],{type:'text/csv;charset=utf-8;'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='censo_docentes_2026_frequencia.csv'; a.click(); URL.revokeObjectURL(url);
}
...
export default function EducacensoDocentes() {
  ...
  const [senha,setSenha]=useState(''); const [desbloqueando,setDesbloqueando]=useState(false); const [sessaoOk,setSessaoOk]=useState(false);
  const [preparandoRf,setPreparandoRf]=useState(''); const [linhaEmEdicao,setLinhaEmEdicao]=useState<Linha|null>(null); const [draft,setDraft]=useState<Draft|null>(null);
  const [fonteMatch,setFonteMatch]=useState(''); const [confirmacao,setConfirmacao]=useState(false); const [enviando,setEnviando]=useState(false); const [resultado,setResultado]=useState<any>(null);
  ...
  const bloquear=async()=>{try{if(token)await bridge({action:'logout'})}catch{}sessionStorage.removeItem(TOKEN_KEY);sessionStorage.removeItem(TOKEN_EXP_KEY);setToken('');setExpiraEm('');setSessaoOk(false);setDraft(null);setLinhaEmEdicao(null);setAviso('Sessão segura do Censo encerrada.')};
  const prepararDireto=async(l:Linha)=>{if(!sessaoOk||!token){setAviso('Desbloqueie primeiro a integração direta segura com sua senha do Diário.');document.getElementById('censo-seguranca')?.scrollIntoView({behavior:'smooth',block:'center'});return;}setPreparandoRf(l.servidor.rf);setAviso('');setResultado(null);setConfirmacao(false);try{const data=await bridge({action:'prepare',rf:dig(l.servidor.rf)}) as PrepareResponse;setLinhaEmEdicao(l);setDraft(construirDraft(l,data));setFonteMatch(data.local?.profileMatch||'ficha consolidada');setTimeout(()=>document.getElementById('censo-direto-editor')?.scrollIntoView({behavior:'smooth',block:'start'}),50);}catch(e:any){setAviso(e?.message||'Não foi possível preparar este vínculo.')}finally{setPreparandoRf('')} };
  ...
  const validarDraft=()=>{if(!draft)return['Cadastro não preparado'];const faltam:string[]=[];const required:Array<[string,string]>=[['Modalidade',draft.modalidade],['CPF',draft.cpf],['Data de nascimento',draft.dataNasc],['Sexo',draft.sexo],['Cor/Raça',draft.corRaca],['Telefone',draft.telefone],['E-mail',draft.email],['Nacionalidade',draft.nacionalidade],['CEP',draft.cep],['Rua',draft.rua],['Bairro',draft.bairro],['Município',draft.municipio],['UF',draft.uf],['Grau de formação',draft.grauFormacao]];required.forEach(([k,v])=>{if(!txt(v))faltam.push(k)});if(draft.nacionalidade==='Estrangeiro(a)'&&!txt(draft.paisEstrangeiro))faltam.push('País de nacionalidade');if(!draft.deficiencias.length)faltam.push('Deficiência/Não possuo deficiência');if(dig(draft.cpf).length!==11)faltam.push('CPF com 11 dígitos');
    if(draft.grauFormacao==='Ensino Superior'){if(!draft.cursosSuperiores.length)faltam.push('Curso superior');draft.cursosSuperiores.forEach((c,i)=>{const n=i+1;if(!c.tipo)faltam.push(`Tipo do ${n}º curso superior`);if(!c.area)faltam.push(`Área do ${n}º curso superior`);if(!c.curso)faltam.push(`Curso do ${n}º curso superior`);if(!c.ufInstituicao)faltam.push(`UF da instituição do ${n}º curso superior`);if(!c.categoriaOrg)faltam.push(`Categoria/Org. do ${n}º curso superior`);if(!c.instituicao)faltam.push(`Instituição do ${n}º curso superior`);});}
    if(draft.possuiPos){if(!draft.posGraduacoes.length)faltam.push('Pós-graduação');draft.posGraduacoes.forEach((p,i)=>{if(!p.tipo)faltam.push(`Tipo da ${i+1}ª pós-graduação`);if(!p.area)faltam.push(`Área da ${i+1}ª pós-graduação`);});}
  ...
  if(loading)return<Loading/>;const expiraLabel=expiraEm?new Date(expiraEm).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';const faltamDraft=draft?validarDraft():[];
  return <div style={{marginTop:4}}>
    <section style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:20,marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap',alignItems:'flex-start'}}><div><h1 style={{margin:0,color:theme.text,fontSize:24}}>👩‍🏫 Censo Docentes 2026</h1><p style={{color:theme.textSecondary,margin:'7px 0 0',maxWidth:850,lineHeight:1.55}}>Campos alinhados ao formulário oficial da Secretaria: frequência + cadastro funcional + ficha cadastral + conferência humana.</p></div><div style={{display:'flex',gap:7,flexWrap:'wrap'}}><span style={{color:theme.primaryText,background:'var(--ghost-bg)',border:`1px solid ${theme.border}`,borderRadius:999,padding:'6px 10px',fontSize:11,fontWeight:850}}>{SCHOOL}</span><span style={{color:sessaoOk?theme.success:theme.warning,background:`${sessaoOk?theme.success:theme.warning}18`,border:`1px solid ${sessaoOk?theme.success:theme.warning}55`,borderRadius:999,padding:'6px 10px',fontSize:11,fontWeight:850}}>{sessaoOk?'🔐 Integração direta segura ativa':'🔒 Integração direta bloqueada'}</span></div></div>
      ...
      {aviso&&<div style={{marginTop:11,padding:10,borderRadius:8,background:'var(--ghost-bg)',border:`1px solid ${theme.border}`,color:theme.text,fontSize:11.5}}>{aviso}</div>}{erro&&<div style={{marginTop:11,padding:10,borderRadius:8,background:`${theme.danger}12`,border:`1px solid ${theme.danger}44`,color:theme.danger,fontSize:11.5}}>{erro}</div>}
    </section>
    <section id="censo-seguranca" style={{background:theme.card,border:`1px solid ${sessaoOk?theme.success:theme.warning}55`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:15,marginBottom:14}}><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}><div><div style={{color:theme.text,fontWeight:900,fontSize:14}}>🔐 Canal direto protegido</div><div style={{color:theme.textSecondary,fontSize:11.5,marginTop:4}}>Os dados são buscados por RF e o envio exige conferência e confirmação final.</div></div>{sessaoOk?<div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><span style={{color:theme.success,fontSize:11.5,fontWeight:800}}>Ativa até {expiraLabel}</span><button style={btn('ghost',{small:true})} onClick={bloquear}>Encerrar sessão segura</button></div>:<div style={{display:'flex',gap:7,alignItems:'center',flexWrap:'wrap'}}><input type="password" autoComplete="current-password" style={{...input,width:220}} value={senha} onChange={e=>setSenha(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void desbloquear()}} placeholder="Senha do Diário"/><button style={btn('primary')} disabled={desbloqueando||!senha} onClick={desbloquear}>{desbloqueando?'Liberando...':'Liberar integração direta'}</button></div>}</div></section>
    ...
    {draft&&linhaEmEdicao&&<section id="censo-direto-editor" style={{marginTop:16,background:theme.card,border:`2px solid ${theme.primaryText}`,borderRadius:theme.radiusMd,boxShadow:theme.shadow,padding:17}}>
      ...
      {draft.grauFormacao==='Ensino Superior'&&<><div style={{marginTop:12,color:theme.textSecondary,fontSize:11.5,fontWeight:850}}>Curso(s) Superior(es) — máximo 3</div><div style={{display:'grid',gap:8,marginTop:7}}>{draft.cursosSuperiores.map((c,i)=><div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:8,padding:10,background:'var(--ghost-bg)'}}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:7}}><SelectField labelText="1º Tipo *" value={c.tipo} onChange={v=>atualizarCurso(i,'tipo',v)} options={TIPOS_SUPERIOR}/><Field labelText="2º Área *" value={c.area} onChange={v=>atualizarCurso(i,'area',v)}/><Field labelText="3º Curso *" value={c.curso} onChange={v=>atualizarCurso(i,'curso',v)}/><SelectField labelText="4º UF Inst. *" value={c.ufInstituicao} onChange={v=>atualizarCurso(i,'ufInstituicao',v)} options={UFS}/><Field labelText="5º Categoria / Org. *" value={c.categoriaOrg} onChange={v=>atualizarCurso(i,'categoriaOrg',v)}/><Field labelText="6º Nome da Instituição *" value={c.instituicao} onChange={v=>atualizarCurso(i,'instituicao',v)}/></div><label style={{display:'flex',gap:7,alignItems:'center',color:theme.textSecondary,fontSize:11,marginTop:8}}><input type="checkbox" checked={c.entregue} onChange={e=>atualizarCurso(i,'entregue',e.target.checked)}/>Cópia entregue nesta UE</label><button style={{...btn('ghost',{small:true}),marginTop:7}} onClick={()=>patch('cursosSuperiores',draft.cursosSuperiores.filter((_,idx)=>idx!==i))}>Remover curso</button></div>)}{draft.cursosSuperiores.length<3&&<button style={btn('ghost',{small:true})} onClick={()=>patch('cursosSuperiores',[...draft.cursosSuperiores,cursoSuperiorVazio()])}>+ Adicionar Curso Superior</button>}</div></>}
      ...
      {draft.possuiPos&&<div style={{display:'grid',gap:8,marginTop:8}}>{draft.posGraduacoes.map((c,i)=><div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:8,padding:10,background:'var(--ghost-bg)'}}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:7}}><SelectField labelText="Tipo *" value={c.tipo} onChange={v=>atualizarPos(i,'tipo',v)} options={TIPOS_POS}/><SelectField labelText="Área *" value={c.area} onChange={v=>atualizarPos(i,'area',v)} options={AREAS_POS}/></div><label style={{display:'flex',gap:7,alignItems:'center',color:theme.textSecondary,fontSize:11,marginTop:8}}><input type="checkbox" checked={c.entregue} onChange={e=>atualizarPos(i,'entregue',e.target.checked)}/>Cópia entregue nesta UE</label><button style={{...btn('ghost',{small:true}),marginTop:7}} onClick={()=>patch('posGraduacoes',draft.posGraduacoes.filter((_,idx)=>idx!==i))}>Remover pós</button></div>)}{draft.posGraduacoes.length<6&&<button style={btn('ghost',{small:true})} onClick={()=>patch('posGraduacoes',[...draft.posGraduacoes,posVazia()])}>+ Adicionar Pós-Graduação</button>}</div>}
