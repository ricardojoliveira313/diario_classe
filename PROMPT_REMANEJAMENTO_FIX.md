...
> **Como o sistema funciona com remanejamento:**
>
> O importador (Importar.tsx) já detecta automaticamente pelo **RA** quando o mesmo aluno aparece
> duas vezes no SED — uma vez como REMA (turma de origem) e outra como ATIVO (turma de destino).
> Ele vincula os dois registros e preenche `turma_origem`, `professora_origem`, `turma_destino`,
> `professora_destino` automaticamente. **Não é necessário nenhum campo extra de data** — os campos
> `data_inicio_matricula` e `data_fim_matricula` já existentes cobrem os dois períodos:
>
> | Registro | Turma | Data início | Data fim |
> |---|---|---|---|
> | REMA (saiu) | 1º Ano A — Roseli | `data_inicio_matricula` | `data_fim_matricula` ← data remanejamento |
> | ATIVO (entrou) | 1º Ano C — Luciane | `data_inicio_matricula` ← nova data | — |
...
| # | O que fazer | Onde |
|---|---|---|
| 0 | **SQL**: adicionar 4 colunas no banco | Supabase SQL Editor |
| 1 | Criar função `labelDocente(nome)` | Antes do export default |
